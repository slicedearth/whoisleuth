...
function base64url(bytes: Uint8Array): string {
  ...
  for (let index = 0; index < bytes.length; index += 3) {
    ...
    const aggregate = (first << 16) | (second << 8) | third;
    encoded += BASE64URL_ALPHABET[(aggregate >>> 18) & 63];
    encoded += BASE64URL_ALPHABET[(aggregate >>> 12) & 63];
    if (index + 1 < bytes.length) encoded += BASE64URL_ALPHABET[(aggregate >>> 6) & 63];
    if (index + 2 < bytes.length) encoded += BASE64URL_ALPHABET[aggregate & 63];
  }
  return encoded;
}
...
function decodeBase64url(value: unknown, expectedBytes: number | null, label: string): Uint8Array {
  ...
  for (const character of value) {
    ...
    if (decoded < 0) throw new Error(`The encrypted workspace archive has an invalid ${label}.`);
