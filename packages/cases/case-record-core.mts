...
export { normalizeDomain } from '../evidence/domain-name.mts';
...
/**
 * Retains an exact submitted hostname only when it is strict evidence input
 * for the Case's canonical registrable identity. This boundary deliberately
 * does not repair URI syntax or infer a hostname from other evidence.
 */
export function normalizeEvidenceHostnameForCase(value: unknown, caseDomain: unknown): string | null {
  const hostname = normalizeEvidenceDomain(value);
  const canonicalCase = normalizeEvidenceDomain(caseDomain);
  if (!hostname || !canonicalCase) return null;
  if (canonicalRegistrableDomain(canonicalCase) !== canonicalCase) return null;
