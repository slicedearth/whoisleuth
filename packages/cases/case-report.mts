...
type CaseReportJson = {
  ...
  analystResponse: {
    ...
    observedEffects: CaseRecord['observedEffects'];
    closures: CaseRecord['closures'];
 ...
 * @returns {object}
 */
function pickKnownSnapshotFields(snapshot: CaseEvidenceSnapshot): ReportSnapshot {
...
export function buildCaseReport(
  ...
  const observedEffects = caseRecord.observedEffects ?? {
    ...
    limitations: ['Observed-effect reviews are independent point-in-time records; provider workflow events do not create or replace them.'],
  };
  const closures = caseRecord.closures ?? {
    records: [],
    omitted: 0,
    preV13HistoryUnavailable: false,
    limitations: ['Closure records are deliberate analyst actions and do not establish absence, safety, provider performance, or legal sufficiency.'],
  };
  ...
  const json: CaseReportJson = {
    ...
    analystResponse: {
      ...
      actions: caseRecord.actions.map((item) => ({
        ...item,
        contactLimitations: [...item.contactLimitations],
        history: item.history.map((event) => ({ ...event, limitations: [...event.limitations] })),
        historyLimitations: [...item.historyLimitations],
      })),
      ...
      observedEffects: {
        reviews: observedEffects.reviews.map((item) => ({ ...item, limitations: [...item.limitations] })),
        omitted: observedEffects.omitted,
        preV13HistoryUnavailable: observedEffects.preV13HistoryUnavailable,
        limitations: [...observedEffects.limitations],
      },
      closures: {
        records: closures.records.map((item) => ({ ...item, limitations: [...item.limitations] })),
        omitted: closures.omitted,
        preV13HistoryUnavailable: closures.preV13HistoryUnavailable,
        limitations: [...closures.limitations],
      },
...
function buildMarkdown(report: CaseReportJson, includeAttribution: boolean): string {
  ...
  if (!response.evidencePins.length && !response.sightings.length && !response.decisions.length && !response.actions.length && !response.assertions.length && !response.manualTrail.length && !response.observedEffects.reviews.length && !response.closures.records.length && !response.branches.length) {
    ...
    for (const action of response.actions) {
      ...
      if (action.providerOutcome) lines.push(`  Latest typed provider outcome: ${escapeMarkdownInline(action.providerOutcome.replaceAll('_', ' '))}`);
      if (action.outcome) lines.push(`  ${action.providerOutcome ? 'Provider' : 'Legacy recorded'} outcome detail: ${escapeMarkdownInline(action.outcome)}`);
      ...
      if (action.historyOmitted) lines.push(`  Earlier transition events omitted by bound: ${action.historyOmitted}`);
      if (action.historyLimitations.length) lines.push(`  History limitations: ${escapeMarkdownInline(action.historyLimitations.join('; '))}`);
    }
    lines.push('');
  ...
  lines.push('### Independent remediation review and closure');
  lines.push('');
  const providerOutcome = report.responseLifecycle.latestProviderOutcome;
  const observedEffect = report.responseLifecycle.latestObservedEffect;
  lines.push(providerOutcome
    ? `- Latest provider outcome time: ${escapeMarkdownInline(providerOutcome.occurredAt)} (${escapeMarkdownInline(providerOutcome.outcome.replaceAll('_', ' '))})`
    : `- Latest provider outcome time: Withheld because the typed event state is ${escapeMarkdownInline(report.responseLifecycle.providerOutcomeState)}.`);
  lines.push(report.responseLifecycle.latestObservedChangeAt
  ...
  for (const review of response.observedEffects.reviews) {
    ...
    if (review.limitations.length) lines.push(`  Limitations: ${escapeMarkdownInline(review.limitations.join('; '))}`);
  }
  if (response.observedEffects.omitted) lines.push(`- Earlier independent reviews omitted by bound: ${response.observedEffects.omitted}`);
