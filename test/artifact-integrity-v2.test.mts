...
describe('sorted JSON artifact compatibility', () => {
  ...
  test('keeps current envelope versions separate from unchanged nested and input contracts', () => {
    ...
    assert.notEqual(BULK_DOMAIN_COMPARISON_EXPORT_VERSION, BULK_DOMAIN_COMPARISON_VERSION);
    assert.notEqual(BULK_MAIL_EXPOSURE_EXPORT_VERSION, BULK_MAIL_EXPOSURE_VERSION);
    assert.notEqual(DOMAIN_CONTROL_MANIFEST_VERSION, DOMAIN_CONTROL_PASSPORT_VERSION);
    assert.notEqual(DOMAIN_CHANGE_PACKET_VERSION, DOMAIN_CHANGE_PACKET_INPUT_VERSION);
  });
