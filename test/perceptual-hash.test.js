...
function icoWrapDib(size) {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      const lum = Math.round((x / (size - 1)) * 255);
...
function icoWrap8bitDib(size) {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      pixels[y * rowSize + x] = Math.round((x / (size - 1)) * (colors - 1)); // gradient of indices
    }
  }
...
describe('faviconPerceptualHash', () => {
  ...
  test('decodes an 8-bit palettized BMP-in-ICO favicon', () => {
    ...
    assert.match(hash, /^[0-9a-f]{16}$/);
    // A left-to-right brightness gradient: every cell is darker than its
    // right neighbour, so no dHash bit is set.
