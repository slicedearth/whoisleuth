...
const SHARED_PRIVACY_FACTS: readonly PrivacyFact[] = Object.freeze([
  ...
  { id: 'portable-file-packages', pattern: /Portable evidence packages process selected JSON, screenshots and opaque files locally.*unchanged, unredacted and unencrypted.*original filenames and paths are not retained.*Unknown source times remain unknown.*review uploads nothing, executes no file and changes no saved records.*separate import preview and confirmation/iu },
  ...
  { id: 'capture-manifest-import', pattern: /capture manifest.*imports only sanitised manifest metadata and.*digests.*artefact bytes are not read or verified by that Case import/iu },
