...
import { evaluateCliFailPolicies, parseCliFailPolicies } from '../cli/fail-policy.mts';
import { runCli } from '../cli/runner.mts';
import EXIT_CODES from '../cli/exit-codes.mts';

describe('explicit CLI failure policies', () => {
  test('parses a bounded distinct policy list', () => {
    assert.deepEqual(parseCliFailPolicies('source-failure,inconclusive,source-failure'), ['source-failure', 'inconclusive']);
    assert.throws(() => parseCliFailPolicies('malicious'), /supports/iu);
