...
describe('buildScanRelationships', () => {
  it('builds distinct, explainable relationship families in deterministic order', () => {
    ...
    assert.deepEqual(result.groups.map((item) => item.type), ['nameserver_set', 'ip_address', 'tracking_identifier', 'tracking_identifier', 'favicon', 'official_asset']);
    ...
    assert.match(result.limitations.join(' '), /require native TLS fingerprints/);
  ...
  it('revalidates and caps externally supplied observation arrays', () => {
    ...
    assert.equal(result.groups.some((item) => ['ip_address', 'tracking_identifier', 'favicon', 'official_asset'].includes(item.type)), false);
    assert.equal(result.truncated, true);
  });
