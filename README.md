...
| **Monitor** | Manage cases, evidence pins, analyst decisions, reviewed response actions and outcomes, response preflight, campaigns, relationship observations, watchlists, a unified retained-evidence timeline, and reports. | Ordinary investigation and response state is browser-local. The timeline projects deliberately retained records without duplicating raw values and keeps observation time separate from storage time. Preflight blocks incomplete incident facts and exposes cautions; it never sends a report. Optional hosted monitoring stores only encrypted compact scheduled-watchlist state. |
