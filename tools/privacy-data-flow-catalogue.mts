...
const FIXTURE_METADATA_PATH = 'packages/contracts/generated/privacy-catalogue-fixture.mts';
...
export function renderPrivacyCatalogueFixtureMetadata(content: string): string {
  const bytes = Buffer.byteLength(content, 'utf8');
  const sha256 = createHash('sha256').update(content).digest('hex');
  return '// Generated by tools/privacy-data-flow-catalogue.mts from the current catalogue bytes.\n'
    + `export const PRIVACY_CATALOGUE_FIXTURE_METADATA = Object.freeze({\n  bytes: ${bytes},\n  sha256: '${sha256}',\n});\n`;
}

function expectedArtifacts(): readonly RetainedArtifact[] {
  const json = renderPrivacyDataFlowCatalogueJson();
  return Object.freeze([
    ...
    Object.freeze({
      path: resolve(ROOT, FIXTURE_METADATA_PATH),
      displayPath: FIXTURE_METADATA_PATH,
      content: renderPrivacyCatalogueFixtureMetadata(json),
    }),
  ]);
}
...
export function main(argv: readonly string[] = process.argv.slice(2)): number {
  ...
  if (drift.length > 0 || fixtureDrift) {
    ...
    process.stderr.write(`Privacy catalogue artefacts are out of date: ${stale.join(', ')}. Run the catalogue writer and review its generated outputs.\n`);
    return 1;
  }
