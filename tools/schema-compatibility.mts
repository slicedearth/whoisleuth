...
const ENTRIES: SchemaCompatibilityEntry[] = [
  ...
  entry({ id: 'cli.investigation-run', kind: 'cli_document', schema: CLI_INVESTIGATION_RUN_SCHEMA, currentVersion: CLI_INVESTIGATION_RUN_VERSION, supportedVersions: [1, CLI_INVESTIGATION_RUN_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'normalize_to_current', writeSemantics: 'normalized_rewrite', byteBudget: MAX_INVESTIGATION_RUN_BYTES, owner: 'cli/investigation-run.mts', note: 'Resumable state for installed fixed-recipe steps; v2 binds bounded analyst selections to exact placeholders while retaining v1 checkpoints, and network work requires per-run approval.' }),
