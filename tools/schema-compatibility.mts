...
import {
  EVIDENCE_STORAGE_MEASUREMENT_FIXTURE_SCHEMA,
  EVIDENCE_STORAGE_MEASUREMENT_FIXTURE_VERSION,
  EVIDENCE_STORAGE_MEASUREMENT_SCHEMA,
  EVIDENCE_STORAGE_MEASUREMENT_VERSION,
  MAX_MEASUREMENT_FIXTURE_BYTES,
} from './evidence-storage-measurement.mts';
import {
...
import { MAX_SIGNER_TRUST_STORE_BYTES, SIGNER_TRUST_STORE_SCHEMA, SIGNER_TRUST_STORE_VERSION, SIGNER_TRUST_REPORT_SCHEMA, SIGNER_TRUST_REPORT_VERSION } from '../cli/signer-trust.mts';
import {
...
import {
  ...
  type SchemaCompatibilityDefinition,
...
import {
  ...
  MAX_INVESTIGATION_MANIFEST_DOCUMENT_BYTES,
...
function entry(value: SchemaCompatibilityDefinition): SchemaCompatibilityEntry {
  const descriptor = defineSchemaCompatibility(value);
  return mutableSchemaCompatibilityEntry(descriptor.tier === 'durable_interchange'
    ? descriptor
    : defineSchemaCompatibility({
        ...descriptor,
        supportedVersions: [descriptor.currentVersion],
        acceptsUnversionedLegacy: false,
      }));
}

function lifecycleEntry(value: SchemaCompatibilityDescriptor): SchemaCompatibilityEntry {
...
const ENTRIES: SchemaCompatibilityEntry[] = [
  ...
  entry({ id: 'maintainer.release-version-check', kind: 'cli_document', schema: RELEASE_VERSION_CHECK_SCHEMA, currentVersion: RELEASE_VERSION_CHECK_VERSION, supportedVersions: [RELEASE_VERSION_CHECK_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'tools/release-version-check.mts', note: 'Local version-alignment and publication-boundary report across package manifests.' }),
  ...
  entry({ id: 'maintainer.source-health', kind: 'cli_document', schema: SOURCE_HEALTH_SCHEMA, currentVersion: SOURCE_HEALTH_VERSION, supportedVersions: [SOURCE_HEALTH_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'tools/source-health.mts', note: 'Offline composition of checked-in retained-source health, reporting-route freshness and reviewed evaluation readiness without refreshing or querying any source.' }),
  ...
  entry({ id: 'tab.investigation-guide', kind: 'tab_store', schema: null, currentVersion: INVESTIGATION_GUIDE_VERSION, supportedVersions: [INVESTIGATION_GUIDE_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'preserve_without_write', migration: 'exact_current_only', writeSemantics: 'ephemeral_replace', byteBudget: MAX_INVESTIGATION_GUIDE_SERIALIZED_BYTES, owner: 'packages/workspace/investigation-guide.mts', note: 'Bounded current recipe state for fixed manual response-preparation workflows without inventing a custom template, action, submission, or stage outcome.' }),
  ...
  entry({ id: 'export.investigation-recipe-summary', kind: 'export', schema: INVESTIGATION_GUIDE_EXPORT_SCHEMA, currentVersion: INVESTIGATION_GUIDE_EXPORT_VERSION, supportedVersions: [INVESTIGATION_GUIDE_EXPORT_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_INVESTIGATION_GUIDE_EXPORT_BYTES, owner: 'packages/workspace/investigation-guide.mts', note: 'The compact current summary retains only workflow metadata, optional template identity, and bounded stage-review reasons.' }),
  entry(LOOKUP_EVIDENCE_COMPATIBILITY),
  ...SCHEMA_LIFECYCLE_REGISTRY.flatMap((family) => family.compatibility.map(lifecycleEntry)),
  ...
  entry({ id: 'cli.command-catalogue', kind: 'cli_document', schema: CLI_COMMAND_CATALOGUE_SCHEMA, currentVersion: CLI_COMMAND_CATALOGUE_VERSION, supportedVersions: [1], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'packages/contracts/cli-command-catalogue.mts', note: 'Dependency-neutral identity for the installed command, usage, collection-mode, and boundary catalogue without target or evidence values.' }),
  ...
  entry({ id: 'cli.mail-review', kind: 'cli_document', schema: CLI_MAIL_REVIEW_SCHEMA, currentVersion: CLI_MAIL_REVIEW_VERSION, supportedVersions: [CLI_MAIL_REVIEW_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_MAIL_REVIEW_INPUT_BYTES, owner: 'cli/mail-review.mts', note: 'The offline passive MX, SPF, and DMARC review retains failed input coverage and original collection provenance independently of report time and display omissions; it never makes a DNS or SMTP connection.' }),
  entry({ id: 'cli.mail-header-review', kind: 'cli_document', schema: CLI_MAIL_HEADER_REVIEW_SCHEMA, currentVersion: CLI_MAIL_HEADER_REVIEW_VERSION, supportedVersions: [CLI_MAIL_HEADER_REVIEW_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_MAIL_HEADER_INPUT_BYTES, owner: 'cli/mail-header-review.mts', note: 'Bounded offline message-header review retains domain-only identity, reported authentication states, and Received routing without body, attachment, local-part, subject, or raw-header output.' }),
  entry({ id: 'cli.workspace-archive-inspection', kind: 'cli_document', schema: ARCHIVE_INSPECTION_SCHEMA, currentVersion: ARCHIVE_INSPECTION_VERSION, supportedVersions: [2, ARCHIVE_INSPECTION_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_OFFLINE_ARTIFACT_BYTES, owner: 'cli/archive-inspect.mts', note: 'Output-only offline archive summary. Version 3 adds a declared v2 content identity while retaining the legacy bare-hash field and comparison. Values remain redacted unless deliberately revealed.' }),
  entry({ id: 'cli.offline-artifact-verification', kind: 'cli_document', schema: OFFLINE_ARTIFACT_VERIFICATION_SCHEMA, currentVersion: OFFLINE_ARTIFACT_VERIFICATION_VERSION, supportedVersions: [OFFLINE_ARTIFACT_VERIFICATION_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_OFFLINE_ARTIFACT_BYTES, owner: 'cli/artifact-verify.mts', note: 'Current verification output preserves separate structure, integrity, projection, and encrypted-envelope assurance.' }),
  entry({ id: 'cli.signed-evidence-package', kind: 'cli_document', schema: SIGNED_EVIDENCE_PACKAGE_SCHEMA, currentVersion: SIGNED_EVIDENCE_PACKAGE_VERSION, supportedVersions: [SIGNED_EVIDENCE_PACKAGE_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'exact_current_only', writeSemantics: 'read_only', byteBudget: MAX_OFFLINE_ARTIFACT_BYTES, owner: 'cli/evidence-signing.mts', note: 'The v1.47.4 current writer signs deterministic sorted-json-v2 bytes; embedded-artifact assurance stays separate.' }),
  ...
  entry({ id: 'cli.evidence-signer-trust-store', kind: 'cli_document', schema: SIGNER_TRUST_STORE_SCHEMA, currentVersion: SIGNER_TRUST_STORE_VERSION, supportedVersions: [SIGNER_TRUST_STORE_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'exact_current_only', writeSemantics: 'read_only', byteBudget: MAX_SIGNER_TRUST_STORE_BYTES, owner: 'cli/signer-trust.mts', note: 'Explicitly selected local public-key fingerprint policy; rotation references and labels cannot confer trust.' }),
  entry({ id: 'cli.evidence-signer-trust-report', kind: 'cli_document', schema: SIGNER_TRUST_REPORT_SCHEMA, currentVersion: SIGNER_TRUST_REPORT_VERSION, supportedVersions: [SIGNER_TRUST_REPORT_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_SIGNER_TRUST_STORE_BYTES, owner: 'cli/signer-trust.mts', note: 'Current fingerprint policy and the selected entry remain separate from cryptographic verification, embedded-artifact assurance and factual truth. No other entries or file paths are emitted.' }),
  ...
  entry({ id: 'cli.cryptographic-assurance-input', kind: 'cli_document', schema: CRYPTOGRAPHIC_ASSURANCE_INPUT_SCHEMA, currentVersion: CRYPTOGRAPHIC_ASSURANCE_INPUT_VERSION, supportedVersions: [CRYPTOGRAPHIC_ASSURANCE_INPUT_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'exact_current_only', writeSemantics: 'read_only', byteBudget: MAX_OFFLINE_EVIDENCE_INPUT_BYTES, owner: 'lib/cryptographic-assurance.mts', note: 'Exact public version 1 source-qualified DNSSEC, route-origin, and DANE or TLSA input remains accepted with independently attributed evidence families.' }),
  ...
  entry({ id: 'cli.bulk', kind: 'cli_document', schema: CLI_BULK_SCHEMA, currentVersion: CLI_BULK_SCHEMA_VERSION, supportedVersions: [CLI_BULK_SCHEMA_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'cli/formatters/json.mts', note: 'Bounded current Bulk result document with compact DNS and passive-mail summaries, nullable per-item observation time, and collection origin.' }),
  entry({ id: 'cli.bulk-item', kind: 'cli_document', schema: CLI_BULK_ITEM_SCHEMA, currentVersion: CLI_BULK_SCHEMA_VERSION, supportedVersions: [CLI_BULK_SCHEMA_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'cli/formatters/json.mts', note: 'One bounded current Bulk JSONL or document item with nullable observation time and explicit collection origin.' }),
  entry({ id: 'cli.bulk-checkpoint', kind: 'cli_document', schema: CLI_BULK_CHECKPOINT_SCHEMA, currentVersion: CLI_BULK_CHECKPOINT_VERSION, supportedVersions: [CLI_BULK_CHECKPOINT_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'exact_current_only', writeSemantics: 'normalized_rewrite', byteBudget: MAX_BULK_CHECKPOINT_BYTES, owner: 'cli/bulk-checkpoint.mts', note: 'Private local compact Bulk checkpoint tied to the exact ordered input digest and scan mode with nullable per-item observation time.' }),
  ...
  entry({ id: 'cli.registry-doctor', kind: 'cli_document', schema: REGISTRY_DOCTOR_SCHEMA, currentVersion: REGISTRY_DOCTOR_VERSION, supportedVersions: [REGISTRY_DOCTOR_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_SAVED_LOOKUP_INPUT_BYTES, owner: 'cli/registry-doctor.mts', note: 'Current output propagates bounded RDAP publication-family truncation and withholds absence or consistency conclusions that omitted values could change.' }),
  ...
  entry({ id: 'cli.domain-assurance-input', kind: 'cli_document', schema: DOMAIN_ASSURANCE_INPUT_SCHEMA, currentVersion: DOMAIN_ASSURANCE_VERSION, supportedVersions: [DOMAIN_ASSURANCE_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'exact_current_only', writeSemantics: 'read_only', byteBudget: MAX_ASSURANCE_INPUT_BYTES, owner: 'lib/domain-assurance.mts', note: 'Bounded current analyst-authored planned-change, recovery-dependency, or retirement input; it performs no credentials or provider changes.' }),
  entry({ id: 'cli.domain-assurance', kind: 'cli_document', schema: DOMAIN_ASSURANCE_SCHEMA, currentVersion: DOMAIN_ASSURANCE_VERSION, supportedVersions: [DOMAIN_ASSURANCE_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'lib/domain-assurance.mts', note: 'Current offline assurance review retains explicit unknown and incomplete states and performs no collection or configuration change.' }),
  ...
  entry({ id: 'cli.domain-change-input', kind: 'cli_document', schema: DOMAIN_CHANGE_INPUT_SCHEMA, currentVersion: DOMAIN_CHANGE_INPUT_VERSION, supportedVersions: [DOMAIN_CHANGE_INPUT_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'exact_current_only', writeSemantics: 'read_only', byteBudget: null, owner: 'lib/domain-change-review.mts', note: 'Bounded analyst-supplied authority, resolver, ACME, certificate, service, and HSTS observations; TXT values are reduced to digests.' }),
  entry({ id: 'cli.domain-change-review', kind: 'cli_document', schema: DOMAIN_CHANGE_REVIEW_SCHEMA, currentVersion: DOMAIN_CHANGE_REVIEW_VERSION, supportedVersions: [1, DOMAIN_CHANGE_REVIEW_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'lib/domain-change-review.mts', note: 'Offline DNS change gate with source timestamps retained independently of generation time, including empty or unavailable snapshots and certificate evidence.' }),
  ...
  entry({ id: 'cli.domain-change-packet', kind: 'cli_document', schema: DOMAIN_CHANGE_PACKET_SCHEMA, currentVersion: DOMAIN_CHANGE_PACKET_VERSION, supportedVersions: SUPPORTED_DOMAIN_CHANGE_PACKET_VERSIONS, acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'lib/domain-change-packet.mts', note: 'Version 3 embeds timestamp-preserving reviews; published version 2 remains verifiable without fabricating absent observation times. Both use deterministic sorted-json-v2 integrity, which does not authenticate their observations.' }),
  ...
  entry({ id: 'cli.workflow-recipe-catalogue', kind: 'cli_document', schema: CLI_WORKFLOW_RECIPE_CATALOGUE_SCHEMA, currentVersion: CLI_WORKFLOW_RECIPE_CATALOGUE_VERSION, supportedVersions: [1], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'cli/investigation-plan.mts', note: 'Request-free installed recipe metadata for list and explanation output; it reads no evidence and executes no step or placeholder.' }),
  entry({ id: 'cli.investigation-run', kind: 'cli_document', schema: CLI_INVESTIGATION_RUN_SCHEMA, currentVersion: CLI_INVESTIGATION_RUN_VERSION, supportedVersions: SUPPORTED_CLI_INVESTIGATION_RUN_VERSIONS, acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'normalize_to_current', writeSemantics: 'normalized_rewrite', byteBudget: MAX_INVESTIGATION_RUN_BYTES, owner: 'packages/contracts/investigation-run.mts', note: 'Version 3 retains incomplete observations without recollection, exact output identities and explicit compatible input bindings. Versions 1 and 2 remain readable; bounded selections bind to remaining placeholders and network work requires per-run approval.' }),
  ...
  entry({ id: 'cli.lookup-brief', kind: 'cli_document', schema: CLI_LOOKUP_BRIEF_SCHEMA, currentVersion: CLI_LOOKUP_BRIEF_VERSION, supportedVersions: [CLI_LOOKUP_BRIEF_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_SAVED_LOOKUP_INPUT_BYTES, owner: 'cli/lookup-brief.mts', note: 'Current offline source-attributed handoff with structured actions and expected outcomes.' }),
  entry({ id: 'cli.registry-cohort', kind: 'cli_document', schema: REGISTRY_COHORT_SCHEMA, currentVersion: REGISTRY_COHORT_VERSION, supportedVersions: [REGISTRY_COHORT_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'exact_current_only', writeSemantics: 'read_only', byteBudget: MAX_REGISTRY_COHORT_INPUT_BYTES, owner: 'cli/registry-cohort.mts', note: 'Current bounded target-free cohort timelines do not treat overlapping samples as independent.' }),
  ...
  entry({ id: 'cli.discover', kind: 'cli_document', schema: CLI_DISCOVER_SCHEMA, currentVersion: CLI_DISCOVER_SCHEMA_VERSION, supportedVersions: [CLI_DISCOVER_SCHEMA_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'cli/formatters/json.mts', note: 'Current candidate discovery document.' }),
  entry({ id: 'cli.discover-item', kind: 'cli_document', schema: CLI_DISCOVER_ITEM_SCHEMA, currentVersion: CLI_DISCOVER_SCHEMA_VERSION, supportedVersions: [CLI_DISCOVER_SCHEMA_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'cli/formatters/json.mts', note: 'One current candidate discovery JSONL item.' }),
  ...
  entry({ id: 'cli.discovery-observation-snapshot', kind: 'cli_document', schema: CLI_DISCOVERY_OBSERVATION_SCHEMA, currentVersion: CLI_DISCOVERY_OBSERVATION_VERSION, supportedVersions: [CLI_DISCOVERY_OBSERVATION_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'exact_current_only', writeSemantics: 'normalized_rewrite', byteBudget: MAX_DISCOVERY_OBSERVATION_BYTES, owner: 'cli/discovery-observation-snapshot.mts', note: 'Private current local registration and DNS baseline with component-specific observation times.' }),
  ...
  entry({ id: 'cli.investigation-manifest', kind: 'cli_document', schema: INVESTIGATION_MANIFEST_SCHEMA, currentVersion: INVESTIGATION_MANIFEST_VERSION, supportedVersions: [...SUPPORTED_INVESTIGATION_MANIFEST_VERSIONS], acceptsUnversionedLegacy: false, futureVersionBehavior: 'reject', migration: 'read_only', writeSemantics: 'read_only', byteBudget: MAX_INVESTIGATION_MANIFEST_DOCUMENT_BYTES, owner: 'packages/investigation/investigation-manifest.mts', note: 'The current path-free manifest identifies JSON and opaque files, declared sources and local packaging events. Exact public v2 manifests remain readable. Digests do not establish signature trust, trusted time or evidence truth.' }),
  ...
  entry({ id: 'cli.cryptographic-assurance-review', kind: 'cli_document', schema: CRYPTOGRAPHIC_ASSURANCE_SCHEMA, currentVersion: CRYPTOGRAPHIC_ASSURANCE_REVIEW_VERSION, supportedVersions: [CRYPTOGRAPHIC_ASSURANCE_REVIEW_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'lib/cryptographic-assurance.mts', note: 'Version 2 emits three independently attributed assurance cards with conservative supplied-row state vocabulary and no combined state, scalar score, or cross-family inference.' }),
  ...
  entry({ id: 'cli.http', kind: 'cli_document', schema: CLI_HTTP_SCHEMA, currentVersion: CLI_HTTP_SCHEMA_VERSION, supportedVersions: [CLI_HTTP_SCHEMA_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'cli/formatters/json.mts', note: 'Current bounded HTTP evidence output includes fixed selected-response delivery metadata and keeps inconclusive activity separate from negative reachability.' }),
  ...
  entry({ id: 'cli.registry-support', kind: 'cli_document', schema: REGISTRY_SUPPORT_SCHEMA, currentVersion: REGISTRY_SUPPORT_SCHEMA_VERSION, supportedVersions: [REGISTRY_SUPPORT_SCHEMA_VERSION], acceptsUnversionedLegacy: false, futureVersionBehavior: 'not_applicable', migration: 'read_only', writeSemantics: 'read_only', byteBudget: null, owner: 'cli/registry-support.mts', note: 'Current catalogue coverage report; optional official lookup links require deliberate browser navigation and do not test live reachability.' }),
...
function snapshotSchemaCompatibilityEntry(value: unknown, index: number): SchemaCompatibilityEntry {
  ...
  return {
    ...
    tier: fields.get('tier') as CompatibilityTier,
