...
test('signs in through the login form and back out again', async ({ page }) => {
  ...
  for (const viewportWidth of [390, 320]) {
    ...
    if (viewportWidth >= 390) await expect(demoLink).toBeVisible();
    else await expect(demoLink).not.toBeVisible();
    const demoBox = viewportWidth >= 390 ? await demoLink.boundingBox() : null;
    ...
    if (viewportWidth >= 390) expect(demoBox).not.toBeNull();
    ...
    const menuTops = [...(demoBox ? [demoBox.y] : []), themeBox!.y, consoleBox!.y, signOutBox!.y];
    const menuBottoms = [...(demoBox ? [demoBox.y + demoBox.height] : []), themeBox!.y + themeBox!.height, consoleBox!.y + consoleBox!.height, signOutBox!.y + signOutBox!.height];
