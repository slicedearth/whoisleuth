...
<section class="rule-builder card">
  <header><div><p class="eyebrow">Custom detection</p><h2>Browser-local rules</h2><p>Combine bounded case-evidence checks without changing the built-in risk model.</p></div><div class="top-actions"><button type="button" onclick={download} disabled={!rules.length}>Export JSON</button><label>Import JSON<input type="file" accept="application/json,.json" onchange={importFile}></label></div></header>
  <form onsubmit={(event)=>{event.preventDefault();create();}}>
    <div class="rule-fields">
      <label>Name<input bind:value={name} maxlength={MAX_RULE_NAME_LENGTH} placeholder="Login page with copied assets" required></label>
      <label>Match<select bind:value={match}><option value="all">All conditions</option><option value="any">Any condition</option></select></label>
      <label>Custom contribution<input type="number" bind:value={riskDelta} min="0" max={MAX_RULE_RISK_DELTA} step="1"></label>
      <label>Suggested tag <small>optional</small><input bind:value={tag} maxlength={MAX_RULE_TAG_LENGTH} placeholder="manual-review"></label>
    </div>
    <fieldset><legend>Conditions</legend>
      {#each conditions as condition,index}
        ...
        <div class="condition-row">
          ...
          {/if}
          <button type="button" class="danger remove-condition" onclick={()=>removeCondition(index)} disabled={conditions.length===1}>Remove</button>
        </div>
      {/each}
      <button type="button" onclick={addCondition} disabled={conditions.length>=MAX_RULE_CONDITIONS}>Add condition</button>
    </fieldset>
  ...
  <section class="rule-list" aria-label="Custom detection rules">
    {#each rules as rule (rule.id)}
      <article class="rule card" class:disabled={!rule.enabled}>
        <header><div><strong>{rule.name}</strong><small>{rule.match==='all'?'All':'Any'} of {rule.conditions.length} condition{rule.conditions.length===1?'':'s'} · {countMatches(rule.id)} current match{countMatches(rule.id)===1?'':'es'}</small></div><div><button type="button" aria-pressed={rule.enabled} onclick={()=>toggle(rule)}>{rule.enabled?'Enabled':'Disabled'}</button><button type="button" class="danger" onclick={()=>remove(rule)}>Delete</button></div></header>
...
{:else}<section class="empty card"><h2>No custom rules yet</h2><p>Create a structured local rule to test existing case evidence without altering the built-in scoring model.</p></section>{/if}

<section class="test-results card">
  <header><div><p class="eyebrow">Current cases</p><h2>Rule test results</h2></div><span>{matchingEvaluations.length} of {records.length} matched</span></header>
  {#if matchingEvaluations.length}
    <ul>{#each matchingEvaluations as result}<li><div><strong>{result.domain}</strong><small>{result.matchedRules.map((item)=>item.name).join(' · ')}</small>{#if result.suggestedTags.length}<small>Suggested: {result.suggestedTags.join(', ')}</small>{/if}</div><div class="scores"><span>Built-in {result.builtInRiskScore??'—'}</span><span>Custom +{result.customRiskDelta}</span><span>Context {result.contextualRiskScore??'—'}</span><button type="button" onclick={()=>openCase(result.caseId)}>Open case</button></div></li>{/each}</ul>
  {:else}<p>No enabled custom rule matches the latest evidence in the current case collection.</p>{/if}
</section>

<style>
  .rule-builder{display:grid;gap:16px;padding:18px}.rule-builder>header,.rule>header,.test-results>header{display:flex;justify-content:space-between;gap:14px;align-items:start}.rule-builder h2,.test-results h2{margin:0}.rule-builder header p,.rule-limits p,.test-results>p{color:var(--muted);font-size:.66rem}.top-actions,.rule>header>div:last-child{display:flex;flex-wrap:wrap;gap:8px}.top-actions label,.top-actions button,.rule button,.condition-row button,.create,fieldset>button,.scores button{min-height:38px;padding:8px 12px;border:1px solid var(--border);border-radius:8px;background:var(--panel-raised);font-size:.68rem}.top-actions label{display:grid;place-items:center;cursor:pointer}.top-actions input{display:none}.rule-builder form{display:grid;gap:14px}.rule-fields{display:grid;grid-template-columns:minmax(180px,1.7fr) repeat(3,minmax(130px,1fr));gap:10px}.rule-fields label,.condition-row label{display:grid;gap:5px;color:var(--muted);font-size:.64rem}.rule-fields input,.rule-fields select,.condition-row input,.condition-row select{min-width:0;min-height:38px;padding:8px 10px;border:1px solid var(--border);border-radius:8px;background:var(--panel)}fieldset{display:grid;gap:10px;margin:0;padding:12px;border:1px solid var(--border);border-radius:9px}legend{padding:0 6px;color:var(--muted);font-size:.66rem}.condition-row{display:grid;grid-template-columns:1.4fr 1fr 1.2fr auto;gap:8px;align-items:end}.remove-condition{align-self:end}.create{justify-self:start}.primary{color:var(--bg);background:var(--accent);border-color:var(--accent);font-weight:700}.danger{color:var(--danger)}.message{color:var(--accent)}.rule-limits{margin:12px 0;padding:14px}.rule-limits p{margin:5px 0 0}.rule-list{display:grid;gap:10px}.rule{display:grid;gap:10px;padding:16px}.rule.disabled{opacity:.62}.rule header>div:first-child{display:grid;gap:3px}.rule header small,.rule li,.rule footer{color:var(--muted);font-size:.64rem}.rule ul{display:grid;gap:5px;margin:0;padding-left:18px}.rule footer{display:flex;flex-wrap:wrap;gap:16px;padding-top:9px;border-top:1px solid var(--border)}.rule footer strong{color:var(--text)}.test-results{display:grid;gap:12px;margin-top:12px;padding:16px}.test-results header>span{color:var(--muted);font-size:.66rem}.test-results ul{display:grid;gap:8px;margin:0;padding:0;list-style:none}.test-results li{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:10px;border:1px solid var(--border);border-radius:8px}.test-results li>div:first-child{display:grid;gap:3px;min-width:0}.test-results small{color:var(--muted);font-size:.62rem;overflow-wrap:anywhere}.scores{display:flex;flex-wrap:wrap;justify-content:end;gap:7px;align-items:center}.scores span{color:var(--muted);font-size:.62rem}.empty{display:grid;min-height:220px;place-content:center;padding:30px;text-align:center}.empty p{color:var(--muted)}
  ...
  @media(max-width:600px){.rule-builder>header,.rule>header,.test-results>header,.test-results li{align-items:stretch;flex-direction:column}.rule-fields,.condition-row{grid-template-columns:1fr}.top-actions>* ,.rule>header>div:last-child button,.create,fieldset>button{width:100%}.scores{justify-content:start}.scores button{width:100%}}
</style>
