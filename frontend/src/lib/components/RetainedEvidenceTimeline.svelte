<script lang="ts">
  ...
  let page = $state(1);
  const filtered = $derived(filterRetainedEvidenceTimeline(timeline, { entity, caseId, source, eventType, time }));
...
<section class="timeline-workspace" aria-labelledby="retained-evidence-timeline-title">
  ...
  <div class="filters" aria-label="Timeline filters">
    ...
    <button type="button" class="btn" onclick={clearFilters} disabled={!entity&&!caseId&&!source&&eventType==='all'&&time==='all'}>Clear filters</button>
  </div>
...
<style>
  ...
  .filters{display:grid;grid-template-columns:repeat(5,minmax(0,1fr)) auto;align-items:end;gap:8px;padding:11px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--panel)}.filters label{display:grid;gap:4px;color:var(--muted);font:650 var(--text-2xs) var(--mono)}.filters select{min-width:0;width:100%}.filters button{min-height:38px}
  ...
  article{display:grid;gap:9px;margin-bottom:11px;padding:13px 14px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--panel)}article>header{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}article h3{margin:3px 0 0;font-size:var(--text-base)}article>p{margin:0;color:var(--muted);font-size:var(--text-xs);line-height:1.5}.kind{color:var(--accent);font:700 var(--text-2xs) var(--mono);text-transform:uppercase}
  .completeness,.truncated{padding:3px 7px;border:1px solid var(--border);border-radius:999px;font:650 var(--text-2xs) var(--mono);text-transform:capitalize}.state-complete{color:var(--accent2)}.state-partial,.truncated{color:var(--amber)}.state-inconclusive{color:var(--danger)}.state-unknown{color:var(--muted)}
