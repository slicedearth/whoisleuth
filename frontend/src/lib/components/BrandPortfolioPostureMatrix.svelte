  ...
  {#if matrix.rows.length}
    ...
    <div class="retained-observations">
      ...
      {#each matrix.rows.filter((row) => row.observationId) as row (row.domain)}
        {@const baseline = active.desiredPostureBaselines.find((item) => item.domain === row.domain)}
        <details id={row.observationId ?? undefined}>
          <summary><span class="observation-heading"><span>{row.domain}</span><strong>{row.observationAt ? date(row.observationAt) : 'No unique latest observation'}</strong></span></summary>
          <p>Expected settings updated {row.baselineUpdatedAt ? date(row.baselineUpdatedAt) : 'at an unavailable time'}.</p>
          {#if baseline}<PostureObservationHistory {baseline} />{/if}
        </details>
      {/each}
...
<style>
  .posture-matrix{display:grid;gap:16px;margin-top:16px;padding:var(--card-pad);min-width:0}
  .posture-matrix h2,.posture-matrix h3{margin:0}
  .section-head p:not(.eyebrow),.empty{max-width:76ch;color:var(--muted);font-size:var(--text-sm);line-height:1.5}
  .scope{white-space:nowrap;border:1px solid var(--border);border-radius:999px;padding:5px 9px;color:var(--muted);font-size:var(--text-xs)}
  .state-summary{display:flex;flex-wrap:wrap;gap:6px}
  .state-summary span{padding:4px 8px;border:1px solid var(--border);border-radius:999px;color:var(--muted);font:600 var(--text-2xs) var(--mono)}
  .matrix-scroll{max-width:100%;overflow:auto;border:1px solid var(--border);border-radius:var(--radius-md)}
  table{width:100%;min-width:1180px;border-collapse:collapse;background:var(--panel)}
  th,td{padding:9px;border-right:1px solid var(--border);border-bottom:1px solid var(--border);text-align:left;vertical-align:top;font-size:var(--text-2xs)}
  ...
  .retained-observations{display:grid;gap:7px}
  .retained-observations>h3{font-size:var(--text-sm)}
  .retained-observations details{scroll-margin-top:16px;padding:10px 12px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--panel-raised)}
  .retained-observations details:target{outline:2px solid var(--accent);outline-offset:2px}
  .retained-observations summary{display:list-item;cursor:pointer;font-size:var(--text-xs)}
  .observation-heading{display:inline-flex;justify-content:space-between;gap:12px;width:calc(100% - 1.2em);vertical-align:top}
  .retained-observations summary span{overflow-wrap:anywhere}
  .retained-observations p,.limitations{color:var(--muted);font-size:var(--text-xs);line-height:1.5}
  .limitations{margin:7px 0 0;padding-left:20px}
  .visually-hidden{position:absolute;width:1px;height:1px;padding:0;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
  @media(max-width:760px){.section-head{display:grid;gap:10px}
  .scope{justify-self:start}
  .matrix-scroll{display:none}
  .mobile-rows{display:grid;gap:10px}
  .mobile-rows article{min-width:0;padding:12px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--panel)}
  .mobile-rows article>header{display:grid;gap:5px}
  .mobile-rows h3,.mobile-rows p,.mobile-rows header span{overflow-wrap:anywhere}
  .mobile-rows p,.mobile-rows header span{margin:3px 0 0;color:var(--muted);font-size:var(--text-2xs)}
  .mobile-rows dl{display:grid;gap:6px;margin:10px 0 0}
  .mobile-rows dl>div{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:8px;padding-top:6px;border-top:1px solid var(--border)}
  .mobile-rows dt,.mobile-rows dd{min-width:0;margin:0;font-size:var(--text-xs)}
  .mobile-rows dd>strong{display:block}
  .mobile-rows dd>span{display:flex;flex-wrap:wrap;gap:6px;margin-top:3px}
  .observation-heading{align-items:flex-start;flex-direction:column;gap:3px}
  }
</style>
