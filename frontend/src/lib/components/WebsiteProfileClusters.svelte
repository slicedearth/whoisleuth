<script lang="ts">
  ...
  let { summary }: { summary: WebsiteProfileClusterSummary } = $props();
  let query = $state('');
  let kind = $state<'all' | 'technology' | 'identity'>('all');
...
<section class="profile-clusters card" aria-labelledby="profile-clusters-title">
  <header class="section-head">
    <div>
      ...
      <p>Find exact technology and page-identity relationships across the latest compact snapshot saved for each domain.</p>
    </div>
  ...
  <div class="filters">
    ...
    <label class="field">Relationship type<select bind:value={kind}><option value="all">All</option><option value="technology">Technology</option><option value="identity">Page identity</option></select></label>
  </div>
  <p class="coverage">Latest snapshots from {summary.domainsReviewed} domain{summary.domainsReviewed === 1 ? '' : 's'} reviewed. Search and grouping remain local to this browser.</p>
  {#if filtered.length}
          ...
          <ul>
            {#each cluster.observations as observation}
              <li>
                ...
                <small>{observation.complete && !observation.truncated ? 'Complete snapshot' : 'Partial snapshot'} · {new Date(observation.observedAt).toLocaleDateString()}</small>
              </li>
            {/each}
          </ul>
...
<style>
  ...
  .cluster-grid>article{min-width:0;padding:13px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-soft)}
