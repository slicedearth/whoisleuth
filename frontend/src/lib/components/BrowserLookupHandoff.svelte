<script lang="ts">
  ...
  function openHandoff() {
    ...
    window.open(preview.destinationUrl, '_blank', 'noopener,noreferrer');
  }
...
<section class="browser-handoff card" aria-labelledby="browser-handoff-title">
  ...
  {#if preview}
    <div class="preview">
      ...
      {/if}
      <div class="actions">
        <button type="button" class="primary" disabled={!disclosureConfirmed} onclick={openHandoff}>{preview.destinationKind === 'lookup' ? 'Open Lookup' : 'Open reviewed destination'}</button>
...
<style>
  ...
  form{display:grid;gap:7px;align-content:start}label{font:700 var(--text-xs) var(--mono)}input,select{min-width:0;width:100%}.configuration{display:grid;grid-template-columns:auto minmax(0,1fr) auto minmax(0,1fr);gap:7px;align-items:center}form small{color:var(--muted);font-size:var(--text-2xs);line-height:1.4}
  ...
  dl{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin:0}dl div{min-width:0;padding:8px;border:1px solid var(--border);border-radius:var(--radius-sm)}dt{color:var(--muted);font:650 var(--text-2xs) var(--mono);text-transform:uppercase}dd{margin:4px 0 0;font-size:var(--text-xs);overflow-wrap:anywhere}.preview p,.preview small{color:var(--muted);font-size:var(--text-xs)}.actions{display:flex;flex-wrap:wrap;gap:8px}.actions button:disabled{cursor:not-allowed;opacity:.5}.consent{display:flex;align-items:flex-start;gap:8px;padding:9px;border:1px solid var(--warning);border-radius:var(--radius-sm);font:var(--text-xs) var(--sans);line-height:1.4}.consent input{width:auto;margin-top:2px}.case-trail{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:7px;align-items:center;padding-top:8px;border-top:1px solid var(--border)}.message{grid-column:1/-1;color:var(--accent);font-size:var(--text-xs)}
  @media(max-width:760px){.browser-handoff{grid-template-columns:1fr}.preview,.message{grid-column:auto}}
  @media(max-width:620px){.configuration,.case-trail,dl{grid-template-columns:1fr}.actions button{flex:1}.configuration label{margin-top:2px}}
</style>
