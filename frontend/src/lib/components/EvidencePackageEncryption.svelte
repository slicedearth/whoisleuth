<script lang="ts">
  ...
  let { encrypted = $bindable(false), disabled = false }: { encrypted?: boolean; disabled?: boolean } = $props();
  ...
  export function takePassphrase(): string | undefined {
    if (!encrypted) { reset(); return undefined; }
...
<div class="encryption-options">
  <label class="choice"><input type="checkbox" checked={encrypted} {disabled} onchange={event => { encrypted = event.currentTarget.checked; reset(); }}> Encrypt package download</label>
  {#if encrypted}
    <div class="passwords">
