...
<section class="form card" aria-busy={busy}><header class="section-head"><h2>{editing ? 'Edit profile' : 'New profile'}</h2><button class="btn" disabled={busy} onclick={close}>Close</button></header>{#if disabledReason}<p class="feature-disabled" role="note">{disabledReason}</p>{/if}<div class="form-grid"><label class="field">Brand name<input id="brand-profile-name" value={values.name} oninput={(event) => setValue('name', event.currentTarget.value)}></label><label class="field">Preferred TLDs<input value={values.tlds} oninput={(event) => setValue('tlds', event.currentTarget.value)}></label><label class="field wide">Official domains<textarea id="official-domains" value={values.official} oninput={(event) => setValue('official', event.currentTarget.value)}></textarea></label><label class="field">Product names<input value={values.products} oninput={(event) => setValue('products', event.currentTarget.value)}></label><label class="field">Mail posture profile<select value={values.mailProtectionProfile} onchange={(event) => setValue('mailProtectionProfile', event.currentTarget.value)}><option value="standard">Active mail</option><option value="defensive_no_mail">Defensive, no mail</option><option value="parked">Parked domain</option></select></label><label class="field">Active DKIM selectors<input value={values.selectors} oninput={(event) => setValue('selectors', event.currentTarget.value)}></label><label class="field">Retired DKIM selectors<input value={values.retiredSelectors} oninput={(event) => setValue('retiredSelectors', event.currentTarget.value)}><small>Retired selectors are checked for continued publication, but are never treated as active keys.</small></label><label class="field wide">Approved partner domains<textarea value={values.partners} oninput={(event) => setValue('partners', event.currentTarget.value)}></textarea></label><label class="field">Trademark owner<input value={values.trademarkOwner} oninput={(event) => setValue('trademarkOwner', event.currentTarget.value)}></label><label class="field">Trademark registration<input value={values.trademarkRegistration} oninput={(event) => setValue('trademarkRegistration', event.currentTarget.value)}></label><BrandIdentityReferencesEditor {officialChannels} {rightsReferences} disabled={busy} onOfficialChannelsChange={setOfficialChannels} onRightsReferencesChange={setRightsReferences}/><fieldset class="wide identity-capture"><legend>Official-site identity</legend><p>Capture a versioned comparison baseline from the first official domain. Only bounded fingerprints and metadata are saved; page HTML is never stored in the profile.</p><div class="identity-actions"><button class="btn" type="button" onclick={captureSiteIdentity} disabled={busy || capturingIdentity || Boolean(disabledReason)}>{capturingIdentity ? 'Capturing…' : pageBaseline ? 'Update official-site baseline' : 'Capture official-site baseline'}</button>{#if pageBaseline}<span>{pageBaseline.domain} · {pageBaseline.complete ? 'Complete' : 'Partial'} · {formatDate(pageBaseline.observedAt)}</span>{:else}<span>Not captured</span>{/if}</div><label class="field">Official favicon hash<input value={values.faviconHash} readonly placeholder="Not captured"></label>{#if pageBaseline}<dl class="baseline-summary stat-grid"><div><dt>Page title</dt><dd>{pageBaseline.pageTitle || 'Not observed'}</dd></div><div><dt>Canonical host</dt><dd>{pageBaseline.canonicalHost || 'Not observed'}</dd></div><div><dt>Page fingerprints</dt><dd>{2 + (pageBaseline.visibleText ? 1 : 0) + (pageBaseline.formStructure ? 1 : 0)} components</dd></div><div><dt>External hosts / tracking IDs</dt><dd>{pageBaseline.resourceHosts.values.length} / {pageBaseline.trackingIdentifiers.values.length}</dd></div></dl>{/if}</fieldset></div><footer class="toolbar"><button class="primary" disabled={busy} onclick={save}>{busy ? 'Saving…' : 'Save profile'}</button><button class="btn" disabled={busy} onclick={close}>Cancel</button></footer></section>
