...
<style>
  ...
  .report-picker{position:relative;flex:0 0 auto;overflow:hidden}.report-picker:focus-within{outline:2px solid var(--focus);outline-offset:3px}.report-picker input{position:absolute;inset:0;opacity:0;cursor:pointer}
  ...
  .source-card dl{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:13px 0 0}.source-card dl>div{min-width:0}.source-card dt{color:var(--muted);font:600 var(--text-2xs) var(--mono)}.source-card dd{margin:3px 0 0;color:var(--text);font-size:var(--text-xs);overflow-wrap:anywhere}
