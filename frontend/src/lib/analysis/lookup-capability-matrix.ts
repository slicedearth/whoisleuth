...
export const LOOKUP_CAPABILITY_ROWS: readonly LookupCapabilityRow[] = Object.freeze([
  ...
  row('registrar-standing', 'Registrar accreditation and compliance context', ['domain'], 'Checked-in IANA registrar IDs and current-year ICANN notices', 'conditional', 'not_collected', 'conditional', 'Requires one unambiguous numeric IANA ID. Provider standing is not evidence that the target is malicious, safe, owned or controlled.'),
  ...
  row('security-txt', 'security.txt disclosure route', ['domain'], 'Exact-host bounded HTTPS request', 'not_collected', 'not_collected', 'conditional', 'Collected only when explicitly selected; publication is not authorisation or safety evidence.'),
