import type { JsonObject, LookupHttpResponse } from './lookup-response.ts';
...
function datedRow(label: string, value: unknown) {
  const datetime = dateTimeAttribute(value);
  return { label, value: formatDate(value), ...(datetime ? { datetime } : {}) };
}

function boundedCredentialCount(value: unknown, maximum = 500): number {
  return Number.isSafeInteger(value) && Number(value) >= 0
    ? Math.min(Number(value), maximum)
    : 0;
}

function boundedPostureCount(value: unknown): number {
  const count = Number(value);
  return Number.isSafeInteger(count) && count >= 0 ? Math.min(count, 20) : 0;
}

function trackingIdentifierLabel(value: unknown): string {
  return (
    {
