...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  ...
  Object.freeze({ id: 'bulk', name: 'Bulk', useWhen: 'You need to compare several candidate domains consistently.', input: 'Paste domains or accept a shortlist from Discover. Choose Fast or Bulk Deep, then choose Gentle, Balanced, or Standard request pacing. Bulk Deep uses compact WHOIS, DNS, website, TLS, and mail evidence rather than the complete single-domain response.', result: 'Fast or Deep checks prioritise candidates and expose related infrastructure already observed in the scan. The focused review cockpit supports explicit row review, case disposition, shortlist, watchlist, and Lookup handoffs. A two-domain comparison keeps source state, observation time, conflict, one-sided evidence, and deliberately unrecorded compact fields distinct. Source, lifecycle, age, mail, registrar, and case filters preserve unknown states. You can save compact sessions to load, compare, export, or resume only domains that never settled.', next: 'Select only the rows you intend to act on, then export, rescan, create cases, set existing case states, or save that selection to Monitor. Open the strongest or most uncertain leads in Lookup, and retain a useful relationship only after reviewing its limitations.' }),
