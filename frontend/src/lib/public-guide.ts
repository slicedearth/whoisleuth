...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; optional security.txt and external intelligence sources run only when selected.', result: 'A source map and lifecycle organize attributed evidence. For domains, a registration authority trace distinguishes the registry publication used for existence decisions from registrar RDAP and WHOIS context. Deep results interpret registration state, disclosure, source agreement and escalation routes without turning partial data into absence. An acquisition workspace can download a bounded local analyst review without making a request or submission. Task and density controls change only the reading view. Bounded reports support domain, IP and ASN results.', next: 'Review conflicting, partial, unavailable and time-sensitive evidence. Choose a task view for the current decision and increase density only when detail is needed. For an open case, retain only the facts needed for comparison. A completed Deep result can also produce a compact website-profile snapshot without another request. Treat an acquisition packet as a checklist record, not proof of eligibility, ownership, price, or transfer. Open an external pivot only after checking its destination and shared value.' }),
