...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; optional security.txt and external intelligence sources run only when selected.', result: 'A bounded evidence map links separately attributed sources to their detail, while an ordered lifecycle shows dated registry and certificate events. Long source records and secondary Deep evidence start collapsed while their headings, states, and summaries remain visible. A separate collapsed panel can open relevant public registration, certificate, history, routing, interconnection, or site-status tools without contacting them in advance.', next: 'Use the source map and lifecycle to review conflicting, partial, unavailable, or time-sensitive evidence. Open an external pivot only after checking the displayed destination and value, then save a useful domain finding to Monitor.' }),
  ...
  Object.freeze({ id: 'monitor', name: 'Monitor', useWhen: 'You want to retain a finding, document a decision or compare later observations.', input: 'Save a case or watchlist from Lookup or Bulk, or retain one reviewed Bulk relationship observation.', result: 'Browser-local timelines, notes, relationship observations and exports keep the review trail together. The activity heatmap covers retained watchlist checks only and does not imply uninterrupted monitoring.', next: 'Review or delete retained relationship observations, rescan deliberately, or use optional hosted monitoring when it is configured.' }),
]);
...
export const guideFaqs: readonly GuideFaq[] = Object.freeze([
  ...
  Object.freeze({ question: 'How do I export or delete saved work?', answer: 'Monitor can export individual cases, and Dashboard can export an encrypted or unencrypted bounded workspace archive and review either format before import. Encrypted passphrases cannot be recovered. Saved browser records can be removed from the tool that stores them or by clearing WHOISleuth site data in your browser.' }),
]);
