...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; optional security.txt and external intelligence sources run only when selected.', result: 'A source map links attributed evidence, while a lifecycle shows registry and certificate events. Deep domain results interpret registration state, disclosure, source agreement and published escalation routes without turning partial data into absence. Task controls reprioritise links; density changes only the reading view. Neither changes collection, evidence, exports, Risk, availability or saved work. Bounded reports support domain, IP and ASN results.', next: 'Review conflicting, partial, unavailable and time-sensitive evidence. Choose a task view for the current decision and increase density only when detail is needed. For an open case, retain only the facts needed for comparison. A completed Deep result can also produce a compact website-profile snapshot without another request. Open an external pivot only after checking its destination and shared value.' }),
  ...
  Object.freeze({ id: 'bulk', name: 'Bulk', useWhen: 'You need to compare several candidate domains consistently.', input: 'Paste domains or accept a shortlist from Discover. Choose Fast or Bulk Deep, then choose Gentle, Balanced, or Standard request pacing. Bulk Deep uses compact WHOIS, DNS, website, TLS, and mail evidence rather than the complete single-domain response.', result: 'Fast or Deep checks prioritise candidates and expose related infrastructure already observed in the scan. The queue shows settled work, percentage, and an estimate based only on the current run. Source, lifecycle, age, mail, registrar, and case filters preserve unknown states. Observed mutation, TLD, registrar, and nameserver groups help narrow a review without claiming common control. You can save compact sessions to load, compare, export, or resume only domains that never settled.', next: 'Select only the rows you intend to act on, then export, rescan, create cases, set existing case states, or save that selection to Monitor. Open the strongest or most uncertain leads in Lookup, and retain a useful relationship only after reviewing its limitations.' }),
  Object.freeze({ id: 'monitor', name: 'Monitor', useWhen: 'You want to retain a finding, document a decision, prepare a reviewed response or compare later observations.', input: 'Save a case or watchlist from Lookup or Bulk, retain selected Lookup facts, save a website profile snapshot, or retain one reviewed Bulk relationship observation.', result: 'A browser-local timeline projects retained case evidence, evidence pins, website snapshots, watchlist checks, and relationship observations without copying raw values. It keeps observation and storage time separate and links each event to its owner. Decisions, actions, notes, relationship views, and exports keep observed evidence separate from analyst reasoning. The activity heatmap covers retained watchlist checks only and does not imply uninterrupted monitoring.', next: 'Filter the timeline by entity, case, source, evidence or change, and time; then open the owning record for exact values and limitations. Review evidence and contact provenance before creating a local response packet. WHOISleuth does not submit the packet.' }),
]);
