...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; Ctrl+Enter or Command+Enter runs the same validated submission as the button. Optional security.txt and external intelligence sources run only when selected.', result: 'At a glance leads into task-ordered evidence families. Every family starts collapsed while its source and limitation counts remain visible. Open one, follow a section link, or use Expand all. Detailed assessment contains the additional decision-support and hand-off panels. Source, relationship, and timeline views organise attributed evidence without changing its state. Partial and unavailable sources remain explicit. Reports cover domains, IPs and ASNs.', next: 'Choose Focus for the decision, then open only the evidence families needed. Review conflicting, partial, unavailable and time-sensitive evidence before retaining a fact. A completed Deep result can create a compact website-profile snapshot, and DNS-health evidence can support a local change rehearsal. Acquisition and change packets remain review checklists rather than proof of eligibility, ownership, price, transfer, or propagation. Check external pivot destinations before opening them.' }),
  ...
  Object.freeze({ id: 'bulk', name: 'Bulk', useWhen: 'You need to compare several candidate domains consistently.', input: 'Paste domains or accept a shortlist from Discover. One job accepts up to 500 Fast or 50 Deep targets. Choose Gentle, Balanced, or Standard request pacing. Bulk Deep uses compact WHOIS, DNS, website, TLS, and mail evidence rather than the complete single-domain response.', result: 'Fast or Deep checks prioritise candidates and expose observed relationships. Bounded maps, coverage bars and an exact two-domain delta table summarise the retained evidence without duplicating it into a second visual encoding. Live counts keep incomplete coverage separate from failure. The review cockpit supports row review, disposition, shortlist, watchlist and Lookup handoffs. Comparisons preserve source state, time, conflicts and one-sided evidence. Saved sessions can be compared, exported or resumed for unsettled domains.', next: 'Select only the rows you intend to act on, then export, rescan, create cases, set existing case states, or save that selection to Monitor. Open the strongest or most uncertain leads in Lookup, and retain a useful relationship only after reviewing its limitations.' }),
  Object.freeze({ id: 'monitor', name: 'Monitor', useWhen: 'You want to retain a finding, document a decision, prepare a reviewed response or compare later observations.', input: 'Save a case or watchlist from Lookup or Bulk, retain selected Lookup facts, save a website profile snapshot or Bulk session, or retain one reviewed relationship observation.', result: 'The review inbox ranks unfinished work and links to its owner. Browser-local timelines project Lookup snapshots, Bulk sessions, watchlist checks, case evidence, pins and relationships without copying raw values. Website pivots group technology and page-identity matches from each domain\'s latest profile. Campaign and cluster counts are not scores or attribution findings. Observation and storage times stay separate, events link to their owners, and analyst reasoning remains distinct from evidence.', next: 'Use the inbox to continue an unfinished case, action or Bulk session, or filter the timeline by area, freshness, entity, case, source, evidence, change and time. Open the owner for exact values and limitations. Review common software or templates before treating a website-pivot match as useful. A stale record is a review cue, not a current finding. Review evidence and contact provenance before creating a local response packet. WHOISleuth does not submit the packet.' }),
]);
...
export const guideFaqs: readonly GuideFaq[] = Object.freeze([
  ...
  Object.freeze({ question: 'Can I review a Lookup evidence export without scanning again?', answer: 'Yes. Lookup can replay a current WHOISleuth evidence JSON file locally after validating its schema and size and calculating its SHA-256 digest. It shows a bounded normalised source and fact summary, does not upload the file or contact the target, and never treats import time as observation time. Raw source payloads in the export are not rendered.' }),
  ...
  Object.freeze({ question: 'How do guided investigations work?', answer: 'Try three resettable offline scenarios in the public Guide, or start a standard guide from Dashboard. Practice uses fictional evidence, makes no requests, and saves nothing. A browser-local template can tailor guidance, omit allowlisted steps, and add approval gates, but cannot run code, start collection, submit evidence, or remove a required gate. An active guide carries the relevant domain or reviewed set between tools. You still start each check and mark its outcome.' }),
  ...
  Object.freeze({ question: 'Why are Deep results collapsed?', answer: 'Evidence families initially stay collapsed so a Deep result does not become one uninterrupted page. Open the families you need or use Expand all. A section link opens its destination. Detailed assessment contains additional decision-support and hand-off panels. Nested RDAP, WHOIS, DNS, HTTP, page, certificate, and network records can remain collapsed. Summaries and source states stay visible. Layout changes never alter collection, scoring, exports, or saved evidence.' }),
