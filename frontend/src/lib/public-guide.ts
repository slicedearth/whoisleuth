...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target. Domain Lookup uses the Deep profile; optional security.txt and external intelligence sources run only when selected.', result: 'Registration evidence and available supporting context are shown by source.', next: 'Review conflicting or incomplete sources, then save a useful domain finding to Monitor.' }),
...
export const guideFaqs: readonly GuideFaq[] = Object.freeze([
  ...
  Object.freeze({ question: 'Should I use Fast or Deep lookup?', answer: 'Single-target Lookup uses Deep collection. Bulk and the CLI offer Fast for lower-request triage or Deep when selected targets merit richer registration, DNS, website, certificate, and network context.' }),
