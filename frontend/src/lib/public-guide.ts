...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  ...
  Object.freeze({ id: 'bulk', name: 'Bulk', useWhen: 'You need to compare several candidate domains consistently.', input: 'Paste domains or accept a shortlist from Discover. One job accepts up to 500 Fast or 50 Deep targets. Choose Gentle, Balanced, or Standard request pacing. Bulk Deep uses compact WHOIS, DNS, website, TLS, and mail evidence rather than the complete single-domain response.', result: 'Fast or Deep checks present Risk, exact source evidence, observed relationships, profile-listed summaries, and a two-domain table. Profile inclusion does not establish protection, ownership, or control. Opportunity is not presented or sortable in Bulk, while existing values remain in saved-session and CSV compatibility contracts.', next: 'Select only the rows you intend to act on, then export, rescan, create cases, set existing case states, or save that selection to Monitor. Open the strongest or most uncertain leads in Lookup, and retain a useful relationship only after reviewing its limitations.' }),
  Object.freeze({ id: 'monitor', name: 'Monitor', useWhen: 'You want to retain a finding, document a decision, prepare a reviewed response or compare later observations.', input: 'Save a case or watchlist from Lookup or Bulk, retain selected Lookup facts, save a website profile snapshot or Bulk session, or retain one reviewed relationship observation.', result: 'The review inbox ranks unfinished work. The evidence-debt matrix uses exact saved Bulk source states and separately pinned case gaps, distinguishing no retained record from an explicit skip. Its queue links to retry, prefilled Deep Lookup, or case review without starting a request. Timelines and retained change review keep source, time, completeness and limits explicit. Missing or partial evidence is not removal. Website pivots and campaign clusters are review aids, not scores or attribution.', next: 'Use the inbox and evidence-debt queue to continue unfinished work, then filter the timeline or choose one retained comparison. Explicitly pair saved Bulk sessions before opening exact rows. A stale record is a review cue, not a current finding. Review common software, source limitations, contact provenance and every target before acting; WHOISleuth does not submit a response or start collection from the queue.' }),
]);
...
export const glossaryTerms: readonly GuideDefinition[] = Object.freeze([
  ...
  Object.freeze({ term: 'Dashboard', definition: 'The signed-in starting page for choosing a Lookup, owned-domain review, candidate review, or acquisition assessment; continuing saved work; or opening a guide.' }),
