...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; optional security.txt and external intelligence sources run only when selected.', result: 'A source map links attributed evidence; a lifecycle shows registry and certificate events. Deep domain results separately interpret EPP state, locks, contact disclosure, publication quality, source reconciliation, and published escalation routes without turning partial data into absence. Long sections start collapsed with state and summaries visible. Bounded readable reports are available for domain, IP, and ASN results. External pivots identify the destination and value before you open them.', next: 'Use the source map and lifecycle to review conflicting, partial, unavailable, or time-sensitive evidence. For an open domain case, retain only the field-level facts needed for a later comparison. After reviewing a completed Deep result, you can also save and compare a compact website-profile snapshot without starting another request. Open an external pivot only after checking the displayed destination and value.' }),
  ...
  Object.freeze({ id: 'monitor', name: 'Monitor', useWhen: 'You want to retain a finding, document a decision, prepare a reviewed response or compare later observations.', input: 'Save a case or watchlist from Lookup or Bulk, retain selected Lookup facts, or retain one reviewed Bulk relationship observation.', result: 'Browser-local timelines, evidence pins and checkpoints, analyst decisions, reviewed actions, notes, relationship observations and exports keep collected evidence separate from analyst reasoning. Response-packet profiles explain what to include, exclude, redact, attach, and follow up for the selected audience. The activity heatmap covers retained watchlist checks only and does not imply uninterrupted monitoring.', next: 'Review evidence and contact provenance, select the intended audience, create a local response packet when the required incident facts are available, record any later outcome, or rescan deliberately. WHOISleuth does not submit the packet.' }),
]);
