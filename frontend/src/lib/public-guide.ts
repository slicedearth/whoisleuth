...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; Ctrl+Enter or Command+Enter runs the same validated submission as the button. Optional security.txt and external intelligence sources run only when selected.', result: 'Source map and lifecycle organise attributed evidence. Stable colours distinguish sources inside visuals; status remains red, amber or green and cards stay neutral. Icons, shapes and labels preserve meaning. Timing, agreement and dependency views avoid repeating detail while retaining exact data. The authority trace separates registry existence from registrar RDAP and WHOIS. Partial and unavailable sources stay explicit. Acquisition review stays local. Reports cover domains, IPs and ASNs.', next: 'Review conflicting, partial, unavailable and time-sensitive evidence. Retain only the facts needed for comparison. A completed Deep result can create a compact website-profile snapshot, and DNS-health evidence can support a local change rehearsal without another request. Acquisition and change packets are checklists, not proof of eligibility, ownership, price, transfer, or successful propagation. Check every external pivot destination and shared value before opening it.' }),
