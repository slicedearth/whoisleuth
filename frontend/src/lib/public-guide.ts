...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  ...
  Object.freeze({ id: 'bulk', name: 'Bulk', useWhen: 'You need to compare several candidate domains consistently.', input: 'Paste domains or accept a shortlist from Discover. Bulk Deep uses compact WHOIS, DNS, website, TLS, and mail evidence rather than the complete single-domain response.', result: 'Fast or Deep checks prioritise candidates and expose related infrastructure already observed in the scan. A bounded Risk/Opportunity matrix summarizes the current filters while the table retains every result. You can explicitly save compact sessions to load, compare, export, or resume only domains that never settled.', next: 'Open the strongest or most uncertain leads in Lookup. If one relationship is useful for later work, retain that bounded observation explicitly.' }),
