...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; optional security.txt and external intelligence sources run only when selected.', result: 'A source map links separately attributed evidence, while a lifecycle shows dated registry and certificate events. Deep domain results separately interpret EPP state, locks, contact disclosure, publication quality, source reconciliation, and published escalation routes without turning partial data into absence. Long sections start collapsed with their state and summary visible. Bounded readable reports are available for domain, IP, and ASN results. External pivots identify the destination and value before you open them.', next: 'Use the source map and lifecycle to review conflicting, partial, unavailable, or time-sensitive evidence. For an open domain case, retain only the field-level facts needed for a later comparison. After reviewing a completed Deep result, you can also save and compare a compact website-profile snapshot without starting another request. Open an external pivot only after checking the displayed destination and value.' }),
