...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; Ctrl+Enter or Command+Enter runs the same validated submission as the button. Optional security.txt and external intelligence sources run only when selected.', result: 'At a glance exposes the exact bounded members behind its complete, limited, disagreement and unresolved counts, then shows at most three task-aware next actions before task-ordered evidence families. A deliberately opened local preview shows at most three matching saved items without another request. Detailed assessment keeps questions and hand-off panels without repeating the action list. Partial and unavailable sources remain explicit. Reports cover domains, IPs and ASNs.', next: 'Choose Focus, then review conflicting, partial, unavailable and time-sensitive evidence before retaining a fact. Risk is secondary explainable triage; open its disclosure for the exact result, factors and sensitivity. Opportunity appears only for acquisition and is readiness context, not availability, value, eligibility, price or likely purchase success. A completed Deep result can create a compact website-profile snapshot. Check external pivot destinations before opening them.' }),
  Object.freeze({ id: 'brands', name: 'Brands', useWhen: 'You want searches and comparisons to reflect an official brand, or you want to assure an owned domain defensively.', input: 'Add official domains, product names, preferred domain endings, trusted infrastructure, known active or retired DKIM selectors, an optional defensive mail profile, and reviewed control-planning context.', result: 'A browser-local profile provides a comparison boundary, bounded public posture audit, external-dependency inventory, separately stored expiring analyst attestations, and a Domain Control Centre for zone intent, lifecycle, recovery dependencies, approved change windows, and portfolio concentration. A reviewed domain-control passport can move only selected owned-domain expectations between the browser and CLI.', next: 'Review the official-domain posture and control-planning gaps, verify any passport before selecting fields to import, or open Discover to generate related candidates.' }),
  Object.freeze({ id: 'discover', name: 'Discover', useWhen: 'You want possible lookalikes, names observed in public certificate logs, or a registry-scoped nameserver pivot.', input: 'Choose a Brand Profile or enter a focused keyword. Use a preset or select exact mutation families; optional custom dictionary terms stay local to the current tab. The separate Nameservers action accepts one hostname and one registry suffix.', result: 'Generated, certificate-log, and nameserver-search candidates retain their source and limits. Nameserver results are a bounded lower bound for one registry, not a global reverse inventory. Internationalised candidates show both their DNS-safe ASCII form and readable Unicode form.', next: 'Review visible cues and source limitations, then form a focused shortlist for Bulk. Sorting changes presentation only; it does not alter evidence, scoring or selection.' }),
  Object.freeze({ id: 'bulk', name: 'Bulk', useWhen: 'You need to compare several candidate domains consistently.', input: 'Paste domains or accept a shortlist from Discover. One job accepts up to 500 Fast or 50 Deep targets. Choose Gentle, Balanced, or Standard request pacing. Bulk Deep uses compact WHOIS, DNS, website, TLS, and mail evidence rather than the complete single-domain response.', result: 'Fast or Deep checks present Risk triage bands, source evidence, relationships, profile summaries, and a two-domain table. Sorting and filtering compare only rows with a compatible current model, ready profile provenance, matching depth and exact source states. Partial, unavailable or incompatible rows remain inconclusive; retained results and factors stay inspectable. Profile inclusion does not establish ownership or control. Opportunity stays hidden in Bulk while retained compatibility remains.', next: 'Select only the rows you intend to act on, then export, rescan, create cases, set existing case states, or save that selection to Monitor. Open the strongest or most uncertain leads in Lookup, and retain a useful relationship only after reviewing its limitations.' }),
  Object.freeze({ id: 'monitor', name: 'Monitor', useWhen: 'You want to retain a finding, document a decision, prepare a reviewed response or compare later observations.', input: 'Save a case or watchlist from Lookup or Bulk, retain selected Lookup facts, save a website profile snapshot or Bulk session, or retain one reviewed relationship observation.', result: 'One stable Monitor route separates Respond views for the inbox, cases, campaigns and relationships from Assure views for timelines, watchlists and local rules. The evidence-debt matrix uses exact saved Bulk source states and separately pinned case gaps. Missing or partial evidence is not removal, and campaign or relationship clusters are review aids rather than attribution.', next: 'Use Respond to continue unfinished case and response work. Use Assure to review retained changes, watchlists and controls. Existing focused links remain within Monitor. Monitor does not start collection, submit a response or apply a control.' }),
]);
...
export const glossaryTerms: readonly GuideDefinition[] = Object.freeze([
  ...
  Object.freeze({ term: 'Dashboard', definition: 'The stable signed-in starting page for Investigate, Respond, or Assure work; continuing saved records; or opening a guide. It is not a fourth analyst job.' }),
...
export const guideFaqs: readonly GuideFaq[] = Object.freeze([
  ...
  Object.freeze({ question: 'Can I review a Lookup evidence export without scanning again?', answer: 'Yes. Lookup can replay a supported WHOISleuth schema 25, 26, 27, or 28 evidence JSON file locally after validating its schema and size and calculating its SHA-256 digest. It shows a bounded normalised source and fact summary, does not upload the file or contact the target, and never treats import time as observation time. Current schema 28 excludes raw registration payloads and registry contact details; legacy schemas remain readable but should be reviewed before sharing.' }),
  ...
  Object.freeze({ question: 'Should I use Fast or Deep lookup?', answer: 'Lookup defaults to Deep for richer registration, DNS, website, certificate and network context. For domains, Deep adds SOA, HTTPS service-binding and effective CAA context, then compares registry, recursive and direct authority evidence. For public IPs, it can add separately attributed PTR names. Direct DNS failures remain inconclusive. Select Fast for registration-first triage. Bulk and the CLI retain explicit Fast and Deep controls.' }),
  Object.freeze({ question: 'How do guided investigations work?', answer: 'Try three resettable offline scenarios, or start one of six fixed guides from Dashboard. Three response guides end at local Case action and packet preflight; they never discover contacts, submit a report, apply a control, or infer an outcome. Templates can tailor allowlisted guidance and add approval gates, but cannot run code, start collection, submit evidence, or remove a required gate. You still start each check and mark its outcome.' }),
  ...
  Object.freeze({ question: 'What does an SSLBL certificate result mean?', answer: 'Eligible Deep domain Lookup compares the observed leaf-certificate fingerprint with a generated local warning-data snapshot and makes no request to SSLBL. An exact match is a review lead tied to that certificate observation, not proof of current domain control or activity. A miss, stale snapshot, or unavailable snapshot is not evidence of safety.' }),
  Object.freeze({ question: 'What does the security.txt option do?', answer: 'When selected for a domain lookup, it requests the standard disclosure file and shows bounded published contacts and policies. The file does not authorise testing or prove that a contact is monitored.' }),
  ...
  Object.freeze({ question: 'Why are Deep results collapsed?', answer: 'Evidence families stay collapsed so a Deep result does not become one uninterrupted page. Open the families you need or use Expand all. A section link opens its destination. Detailed assessment contains questions, claim-readiness and hand-off panels without repeating At a glance actions. Nested RDAP, WHOIS, DNS, HTTP, page, certificate, and network records can remain collapsed. Summaries and source states stay visible. Layout changes never alter collection, scoring, exports, or saved evidence.' }),
  Object.freeze({ question: 'Can I compare how a website changes over time?', answer: 'Yes. Save a reviewed Deep Lookup as a compact website snapshot, then compare it with a later snapshot. WHOISleuth keeps curated technology, posture, identity digests, source health, completeness and optional normalised leaf-certificate evidence, not raw page, registration or certificate payloads. The local inventory shows the latest saved certificate per domain. A difference or shared exact fingerprint is a review lead, not proof of compromise, ownership, common control, intent or maliciousness.' }),
  Object.freeze({ question: 'Can I save a relationship found in Bulk?', answer: 'Yes. Choose Retain observation on a reviewed relationship group. WHOISleuth saves only the bounded normalised value, member domains, method, time, completeness and limitations in this browser. It does not retain the complete Bulk scan or raw responses. Review or delete it under Monitor Relationships.' }),
