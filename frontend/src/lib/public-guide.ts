...
export const toolGuides: readonly GuideEntry[] = Object.freeze([
  Object.freeze({ id: 'lookup', name: 'Lookup', useWhen: 'You have one domain, IP address or ASN to investigate.', input: 'Enter one target and choose Fast or Deep. Deep is the default; optional security.txt and external intelligence sources run only when selected.', result: 'A source map links separately attributed evidence, while a lifecycle shows dated registry and certificate events. Deep domain results separately interpret EPP state, locks, contact disclosure, publication quality, source reconciliation, and published escalation routes without turning partial data into absence. Long sections start collapsed with their state and summary visible. External pivots identify the destination and value before you open them.', next: 'Use the source map and lifecycle to review conflicting, partial, unavailable, or time-sensitive evidence. After reviewing a completed Deep result, you can explicitly save and compare compact website-profile snapshots without starting another request. Open an external pivot only after checking the displayed destination and value, then save a useful domain finding to Monitor.' }),
...
export const guideFaqs: readonly GuideFaq[] = Object.freeze([
  ...
  Object.freeze({ question: 'How do guided investigations work?', answer: 'Start a guide from the Dashboard. It shows one step at a time, keeps the active target, Brand Profile, matching case, and next manual action visible, explains what a network step may request, and carries the relevant domain or bounded reviewed set between tools. Bulk comparison results can enter a Monitor review queue, but cases are created only when you explicitly open them. You still start each check and mark the step reviewed, partial, or skipped.' }),
