...
{:else}<div class="shell" class:open={navOpen}><header><a href="/"><span class="mark small">W</span><strong>WHOISleuth</strong></a><button aria-label="Toggle navigation" aria-expanded={navOpen} aria-controls="workspace-navigation" onclick={toggleNavigation}>☰</button></header><aside id="workspace-navigation"><a class="brand" href="/"><span class="mark">W</span><span><strong>WHOISleuth</strong><small>Domain intelligence</small></span></a><nav aria-label="Workspaces"><p class="eyebrow">Workspaces</p>{#each workspaces as item}<a class:active={page.url.pathname===item.href} aria-current={page.url.pathname===item.href?'page':undefined} href={item.href} onclick={()=>navOpen=false}><strong>{item.label}</strong><small>{item.detail}</small></a>{/each}</nav><div class="session">Session active <button onclick={logout}>Sign out</button></div></aside>{#if navOpen}<button class="scrim" aria-label="Close navigation" onclick={()=>navOpen=false}></button>{/if}<main id="main-content" tabindex="-1">{@render children()}</main></div>{/if}
<style>:global(html){scroll-behavior:smooth}:global(button),:global(input),:global(select),:global(textarea){font:inherit}:global(button),:global(a),:global(input),:global(select),:global(textarea),:global(summary){outline-offset:3px}:global(:focus-visible){outline:2px solid #9cf4e6}:global(button:disabled){cursor:not-allowed;opacity:.58}:global(main header){display:flex}@media(max-width:900px){:global(main header){position:static;inset:auto;z-index:auto;height:auto;padding:0;border:0;background:transparent}:global(main header button){width:auto;height:auto}}@media(prefers-reduced-motion:reduce){:global(html){scroll-behavior:auto}:global(*),:global(*::before),:global(*::after){scroll-behavior:auto!important;transition-duration:.01ms!important;animation-duration:.01ms!important;animation-iteration-count:1!important}}</style>
