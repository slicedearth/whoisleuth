<script lang="ts">
  ...
  function generate() {
    ...
    if (mode === 'keyword') {
      const next = generateKeywordCandidates();
      setResults(next, `Generated ${next.length} naming candidates.`);
      return;
    }
    const generated = generateTyposquatCandidates(seed, tlds()) as Array<{domain:string;source:string;mutationTypes:string[]}>;
    setResults(generated, `Generated ${generated.length} explainable lookalike variants.`);
  }
  ...
  async function searchCt() {
    ...
    try {
      ...
      setResults(next, `Found ${next.length} unique hostnames from ${body.certCount || 0} certificates${body.truncated ? ' (result cap reached)' : ''}.`);
    } catch (cause) {
