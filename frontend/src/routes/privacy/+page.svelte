...
<article class="policy card">
  ...
  <p>Requests may reach the relevant registry or registrar, public DNS, a domain's MTA-STS policy host, <a href="https://crt.sh/" target="_blank" rel="noopener">crt.sh</a>, and the infrastructure hosting this deployment. When the optional archived-verdict adapter is configured and explicitly selected, <a href="https://urlscan.io/privacy/" target="_blank" rel="noopener">URLscan</a> receives the canonical registrable domain and ordinary API request metadata for a search-only request under its own terms, privacy policy, and account quota. When the optional malware-host adapter is configured and explicitly selected, <a href="https://abuse.ch/privacy-policy/" target="_blank" rel="noopener">URLhaus</a> receives the same bounded domain target and ordinary request metadata for a host lookup under its own fair-use terms, privacy policy, and account quota. Neither integration submits a target for scanning or reporting. If distributed operation limits are enabled, their configured REST service processes the minimal lease and optional fixed-window counter metadata described above. No analytics, advertising, or CRM processors are used.</p>
