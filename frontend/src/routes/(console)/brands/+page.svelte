<script lang="ts">
  ...
  async function retainObservation(report:DomainPostureHttpResponse){if(!active)return;const baseline=active.desiredPostureBaselines.find((item)=>item.domain===report.domain);if(!baseline){message='Configure a desired posture baseline before retaining an observation.';return;}await saveBaselines(active.desiredPostureBaselines.map((item)=>item.domain===report.domain?{...item,previousObservation:buildDesiredPostureObservation(report),updatedAt:new Date().toISOString()}:item));message=`Retained the compact ${report.checkedAt} posture observation for ${report.domain}.`;}
...
{#if active}<BrandPostureAudit {active} disabledReason={postureReason} {auditing} results={auditResults} {audit} {retainObservation} /><BrandDesiredPostureBaselines {active} {saveBaselines} /><BrandProtectionAttestations {active} {saveAttestations} />{/if}
