...
<div class="policy-layout">
  ...
  <article class="policy card" aria-labelledby="privacy-title">
    ...
    <p id="privacy-title"><strong>Last updated: 24 August 2026.</strong></p>
    ...
    <p><strong>Compatibility.</strong> Current Case schema 13 can retain the exact normalised submitted hostname on a new evidence snapshot. Exact public Case schema 12 remains readable and migrates directly; migrated history can retain a null hostname because WHOISleuth does not reconstruct one from weaker fields. Case report v9 JSON and Markdown do not add that hostname. Public CLI case packs clear identifiers, actions, observed-effect reviews, and closure records; ordinary Case, workspace, trusted, and internal files can contain exact investigated hostnames and analyst context.</p>
    ...
    <p>The current writer emits workspace archive version 6. Exact version 5 remains readable with an explicitly empty migrated Analyst Review Item section. Versions 1 through 4 and future versions fail without empty import, reset, deletion, or rewrite. Release 1.47.4 can export the exact version-5 and Case-schema-12 public baseline before moving to v2.</p>
    ...
    <p><strong>Sensitive files.</strong> A full saved Lookup can contain targets, bounded source endpoints and timings, raw RDAP publications, WHOIS response bodies, and publicly published contacts. Current Lookup evidence schema 27 excludes raw registration payloads, expanded contacts, credentials, and complete query-bearing URLs; exact public schema 26 remains readable and may contain public contact fields. Case, workspace, graph, campaign, response, and defensive files can identify investigated targets or contain analyst material. Review every file before sharing.</p>
