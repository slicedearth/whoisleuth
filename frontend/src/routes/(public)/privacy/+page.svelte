...
<div class="policy-layout">
  ...
  <article class="policy card" aria-labelledby="privacy-title">
    ...
    <p><strong>Authorised local capture.</strong> The optional capture companion, separate from the hosted application and main CLI, executes remote page JavaScript in a disposable network-bounded browser. Each admitted resource operator receives the exact requested URL, including its path and query. Structured output has no dedicated path or query fields, but the page-controlled title can reproduce them. A local fixed-size screenshot necessarily preserves visible rendered content and may include page text or a reflected path or query. Captures are not uploaded to WHOISleuth and persist until the operator deletes them. When an analyst selects a capture manifest for a Case, the browser validates it before preview and imports only sanitised manifest metadata and declared digests; screenshot, DOM-digest, and other capture artefact bytes are not read or verified by that Case import. Selecting them for a portable evidence package is a separate local file action.</p>
