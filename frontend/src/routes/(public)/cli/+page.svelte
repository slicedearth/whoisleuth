  ...
  <div class="handoff-recipes independent-grid">
    {#each PUBLIC_CLI_GUIDANCE.interchangeRecipes as recipe,index}
      <article>
        ...
        <div class="recipe-commands">{#each recipe.commands as command}<CopyableCommand {command} label={`${recipe.label} command`} compact />{/each}</div>
        <p>{recipe.result}</p>
      </article>
    {/each}
  </div>
...
<section class="cli-section" id="more" aria-labelledby="more-title">
  ...
  <aside class="package-verification">
    <div><h3>Verify the release you intended to install</h3><p>Application {__WHOISLEUTH_VERSION__} · Node.js 24 or later. {PUBLIC_CLI_GUIDANCE.verification[0]}</p></div>
    <nav aria-label="Release evidence">
      <a href={`${WHOISLEUTH_SOURCE_REPOSITORY_URL}/releases`} target="_blank" rel="noopener">Reviewed releases<span class="sr-only"> (opens in a new tab)</span></a>
      <a href="https://www.npmjs.com/package/@slicedearth/whoisleuth-cli" target="_blank" rel="noopener">npm integrity and provenance<span class="sr-only"> (opens in a new tab)</span></a>
      <a href="https://api.github.com/repos/slicedearth/whoisleuth/dependency-graph/sbom" target="_blank" rel="noopener">Repository SBOM<span class="sr-only"> (opens in a new tab)</span></a>
      <a href={`${WHOISLEUTH_SOURCE_REPOSITORY_URL}/blob/main/packages/cli/README.md`} target="_blank" rel="noopener">Package guide<span class="sr-only"> (opens in a new tab)</span></a>
    </nav>
  </aside>
</section>
</PublicReferenceDocument>
