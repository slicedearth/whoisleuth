  ...
  <section id="result">
    ...
    {#if httpEvidence.source==='http'}
      <section class="http-card card" aria-labelledby="http-title">
        ...
        {#if httpResponse.status}<details class="http-detail"><summary>Selected response headers</summary><dl>{#each httpSecurityRows() as row}<dt>{row[0]}</dt><dd>{show(row[1])}</dd>{/each}<dt>Server</dt><dd>{show(httpResponse.server)}</dd><dt>Content language</dt><dd>{show(httpResponse.contentLanguage)}</dd><dt>Declared length</dt><dd>{httpResponse.declaredContentLength===null||httpResponse.declaredContentLength===undefined?'—':formatBytes(httpResponse.declaredContentLength)}</dd></dl></details>{/if}
...
<style>
  ...
  @media(max-width:650px){.input-row,.summaries,.sources,.score-details,.response-actions,.idn-forms,.dns-grid,.http-grid{grid-template-columns:1fr}.http-detail dl{grid-template-columns:1fr}.input-row .primary{min-height:44px}.result-head{align-items:flex-start;flex-direction:column}.result-actions{width:100%;flex-wrap:wrap}.scores{display:grid;grid-template-columns:1fr 1fr}.score{width:auto}}
</style>
