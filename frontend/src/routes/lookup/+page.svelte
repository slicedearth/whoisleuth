...
{#if result}
  <section id="result">
    ...
    {#if tlsEvidence.source==='tls'}
      <section class="tls-card http-card card" aria-labelledby="tls-title">
        <header><div><p class="eyebrow">Deep-scan evidence</p><h3 id="tls-title">TLS and certificate intelligence</h3></div><span class:partial={!tlsEvidence.complete}>{statusLabel(show(tlsEvidence.status))}</span></header>
        <div class="tls-grid http-grid">
    ...
    {#if pageComparison}
      <section class="page-comparison card" aria-labelledby="page-comparison-title">
        ...
        <p class="page-comparison-note">Each component stands on its own. WHOISleuth does not combine these observations into a page-similarity score or use them to change the Risk score. Matches can arise from shared templates, providers, libraries, or analytics, and do not prove common ownership, copying, intent, or maliciousness.</p>
      </section>
    {:else if profile?.pageBaseline && result.type==='domain'}
      <section class="page-comparison unavailable-comparison card" aria-labelledby="page-comparison-title">
        <header><div><p class="eyebrow">Active Brand Profile</p><h3 id="page-comparison-title">Official-site comparison</h3></div><span class="partial">Unavailable</span></header>
        <p>No current compatible page fingerprint was captured, so the saved official-site baseline cannot be compared with this result. This does not indicate that the pages differ.</p>
      </section>
    {/if}
    ...
    <div class="sources">
      <details class="card" open><summary>RDAP structured data</summary>{#if rdap.error}<p class="error source-error">{rdap.error}</p>{:else}
        ...
        {:else}
          {#if rdapParsed.serverTruncated}<p class="source-partial"><strong>Server-declared partial response.</strong> The registry reported that some RDAP data was omitted.{Array.isArray(rdapParsed.serverTruncationReasons)&&rdapParsed.serverTruncationReasons.length?` ${rdapParsed.serverTruncationReasons.join(' · ')}.`:''}</p>{/if}
          <dl>
    ...
    {#if threatIntelligenceProviders.length}
      <section class="threat-intelligence card" aria-labelledby="threat-intelligence-title">
        ...
        {#each threatIntelligenceProviders as provider}
          ...
          <article>
            <div class="threat-source"><strong>{show(providerIdentity.label)}</strong><span class:error-state={provider.state==='error'||provider.state==='unavailable'||provider.state==='rate_limited'}>{show(provider.state)}</span></div>
            {#if provider.detail}<p>{show(provider.detail)}</p>{/if}
            {#if findings.length}<ul>{#each findings as finding}<li><div><strong>{show(finding.category)}</strong><span>{[finding.providerVerdict,finding.lastObservedAt?formatDate(finding.lastObservedAt):null].filter(Boolean).join(' · ')}</span></div>{#if finding.detail}<p>{show(finding.detail)}</p>{/if}{#if finding.referenceUrl}<a href={finding.referenceUrl} target="_blank" rel="noopener">View attributed provider record</a>{/if}</li>{/each}</ul>{/if}
            {#if Array.isArray(providerObservation.limitations)&&providerObservation.limitations.length}<details><summary>Limitations</summary><ul>{#each providerObservation.limitations as limitation}<li>{show(limitation)}</li>{/each}</ul></details>{/if}
          </article>
        {/each}
      </section>
    {/if}
...
<style>
  ...
  .tls-grid .danger-text{color:var(--danger)}.tls-findings{display:grid;gap:7px;margin:10px 0 0;padding:0;list-style:none}.tls-findings li{padding:8px 10px;border-left:3px solid var(--accent);background:rgba(126,224,168,.04)}.tls-findings li.danger{border-color:#f2b84b;background:rgba(242,184,75,.04)}.tls-findings strong,.tls-findings span{display:block;overflow-wrap:anywhere}.tls-findings strong{font-size:.65rem}.tls-findings span{margin-top:3px;color:var(--muted);font-size:.62rem}
  ...
  @media(max-width:650px){.threat-intelligence>header,.threat-source,.threat-intelligence li>div{display:block}.threat-intelligence>header>span,.threat-source>span,.threat-intelligence li span{display:block;margin-top:4px}}
</style>
