      ...
      <details class="card" open><summary>RDAP structured data</summary>{#if rdap.error}<p class="error source-error">{rdap.error}</p>{:else}
        <dl>
          {#if result.type==='domain'}
            <dt>Domain</dt><dd>{show(rdapParsed.domain)}</dd><dt>Unicode name</dt><dd>{show(rdapParsed.unicodeDomain)}</dd><dt>Registry ID</dt><dd>{show(rdapParsed.handle)}</dd><dt>Registrar</dt><dd>{show(rdapParsed.registrar)}</dd><dt>Registrar ID</dt><dd>{show(rdapParsed.registrarIanaId)}</dd><dt>DNSSEC</dt><dd>{show(rdapParsed.dnssec)}</dd><dt>Status</dt><dd>{show(rdapParsed.statuses)}</dd><dt>Nameservers</dt><dd>{show(rdapParsed.nameservers)}</dd><dt>IDN variants</dt><dd>{rdapVariantText()||'—'}{rdapParsed.variantsTruncated?' (capped)':''}</dd>
          {:else if result.type==='ipv4'||result.type==='ipv6'}
            <dt>Handle</dt><dd>{show(rdapParsed.handle)}</dd><dt>Name</dt><dd>{show(rdapParsed.name)}</dd><dt>Range</dt><dd>{show(rdapParsed.startAddress)} – {show(rdapParsed.endAddress)}</dd><dt>CIDRs</dt><dd>{show(rdapParsed.cidrs)}</dd><dt>Country</dt><dd>{show(rdapParsed.country)}</dd><dt>Type</dt><dd>{show(rdapParsed.networkType)}</dd>
          {:else if result.type==='asn'}
            <dt>Handle</dt><dd>{show(rdapParsed.handle)}</dd><dt>Name</dt><dd>{show(rdapParsed.name)}</dd><dt>AS range</dt><dd>{show(rdapParsed.startAutnum)} – {show(rdapParsed.endAutnum)}</dd><dt>Country</dt><dd>{show(rdapParsed.country)}</dd><dt>Type</dt><dd>{show(rdapParsed.autnumType)}</dd>
          {/if}
          <dt>Object class</dt><dd>{show(rdapParsed.objectClassName)}</dd><dt>Language</dt><dd>{show(rdapParsed.language)}</dd><dt>Conformance</dt><dd>{show(rdapParsed.conformance)}</dd><dt>Port 43</dt><dd>{show(rdapParsed.port43)}</dd><dt>Parent handle</dt><dd>{show(rdapParsed.parentHandle)}</dd><dt>Redactions</dt><dd>{rdapRedactionText()||'—'}{rdapParsed.redactionsTruncated?' (capped)':''}</dd><dt>Links</dt><dd>{rdapLinkText()||'—'}</dd><dt>Notices</dt><dd>{rdapTextBlocks(rdapParsed.notices)||'—'}</dd><dt>Remarks</dt><dd>{rdapTextBlocks(rdapParsed.remarks)||'—'}</dd>
        </dl>
...
<style>
  ...
  @media(max-width:650px){.input-row,.summaries,.sources,.score-details,.response-actions{grid-template-columns:1fr}.input-row .primary{min-height:44px}.result-head{align-items:flex-start;flex-direction:column}.result-actions{width:100%;flex-wrap:wrap}.scores{display:grid;grid-template-columns:1fr 1fr}.score{width:auto}}
</style>
