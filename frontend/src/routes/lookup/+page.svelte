  ...
  <section id="result">
    ...
    {#if pageIdentity.source==='html'}
      <section class="page-card card" aria-labelledby="page-identity-title">
        ...
        <p>Bounded metadata from the static HTML already captured for this lookup. Resource and embedded locations retain origins only; contact links retain domains only; download paths and URL queries are not retained. These fields provide relationship and review context rather than proof of ownership or maliciousness.</p>
      </section>
    {/if}
...
<style>
  ...
  @media(max-width:650px){.input-row,.summaries,.sources,.score-details,.response-actions,.idn-forms,.dns-grid,.http-grid,.page-grid{grid-template-columns:1fr}.http-detail dl,.page-detail dl{grid-template-columns:1fr}.input-row .primary{min-height:44px}.result-head{align-items:flex-start;flex-direction:column}.result-actions{width:100%;flex-wrap:wrap}.scores{display:grid;grid-template-columns:1fr 1fr}.score{width:auto}}
</style>
