...
async function checkMxRecords(domain) {
  try {
    ...
    // RFC 7505 "null MX" - a single record pointing at the root domain (".")
    // is a domain explicitly declaring it accepts no mail, not "mail is
    // configured". Node's resolver strips the trailing dot from the target,
    // so the root comes back as "" as well as ".".
    const realRecords = records.filter((r) => r.exchange !== '.' && r.exchange !== '');
    // Distinct from simply having no MX record at all - a null MX is a
    // deliberate "this domain never accepts mail" declaration, not an
    // absence of configuration. Not currently used for scoring (both cases
    // already correctly score as hasMx: false); exposed for callers that
...
module.exports = { checkEmailSecuritySignals };
