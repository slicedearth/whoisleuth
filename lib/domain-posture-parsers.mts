...
function joinTxtRecords(records: unknown): string[] {
...
function parseTagList(record: unknown): TagList {
...
function parseMtaStsDnsRecords(records: unknown) {
  const allRecords = joinTxtRecords(records);
  const policyRecords = allRecords.filter((record) => /^v\s*=\s*stsv1(?:\s*;|$)/i.test(record));
  const result: { records: string[]; valid: boolean; record: string | null; id: string | null; issues: string[] } = { records: policyRecords, valid: false, record: policyRecords[0] || null, id: null, issues: [] };
...
function parseMtaStsPolicy(text: unknown) {
...
function parseBimiRecords(records: unknown) {
  const allRecords = joinTxtRecords(records);
  const bimiRecords = allRecords.filter((record) => /^v\s*=\s*bimi1(?:\s*;|$)/i.test(record));
  const result: { records: string[]; valid: boolean; record: string | null; logo: string | null; authority: string | null; issues: string[] } = { records: bimiRecords, valid: false, record: bimiRecords[0] || null, logo: null, authority: null, issues: [] };
