...
function normalizeDkimSelectors(rawSelectors) {
  ...
  return [...new Set(rawSelectors
    .map((selector) => String(selector || '').trim().toLowerCase().replace(/^\.+|\.+$/g, ''))
...
function tlsRptCheck(query) {
  if (query.error) return queryFailureCheck('tls_rpt', 'TLS-RPT', query.error);
  const parsed = parseTlsRptRecords(query.records);
  if (parsed.records.length === 0) {
    return check('tls_rpt', 'TLS-RPT', 'warning', 'No SMTP TLS reporting policy', {
      remediation: 'Publish a v=TLSRPTv1 record at _smtp._tls with a monitored rua destination.',
    });
  }
  if (!parsed.valid) {
...
async function fetchMtaStsPolicy(domain) {
  ...
  try {
    ...
    if (!res.ok) return { text: '', contentType: res.headers.get('content-type'), error: `Policy endpoint returned HTTP ${res.status}.` };
