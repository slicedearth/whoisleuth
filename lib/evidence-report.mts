...
const MAX_REPORT_RISK_FACTORS = 20;
...
function riskReport(value: unknown) {
  const source = objectOrEmpty(value);
  const score = Number.isInteger(source.score) && Number(source.score) >= 0 && Number(source.score) <= 100
    ? Number(source.score)
    : null;
  const modelVersion = Number.isInteger(source.modelVersion)
    && Number(source.modelVersion) > 0
    && Number(source.modelVersion) <= 10_000
    ? Number(source.modelVersion)
    : null;
  const factors = (Array.isArray(source.factors) ? source.factors : [])
    .slice(0, MAX_REPORT_RISK_FACTORS)
    .map((value) => {
      const factor = objectOrEmpty(value);
      const label = cleanReportText(factor.label, '');
      const deltaValue = factor.delta ?? factor.points;
      const delta = Number.isInteger(deltaValue) && Math.abs(Number(deltaValue)) <= 100
        ? Number(deltaValue)
        : null;
      if (!label || delta === null) return null;
      return `${label} (${delta > 0 ? '+' : ''}${delta})`;
    })
    .filter((value): value is string => value !== null);
  return { score, modelVersion, factors };
}
...
function buildLookupEvidenceReport(
  document: unknown,
  ...
  const groups = {
    registryRdap: {
      ...
      fields: [
        ...
        reportField('Server-declared truncation', yesNoUnknown(rdapParsed.serverTruncated)),
      ],
    },
    registrarRdap: {
      title: 'Registrar RDAP',
      fields: [
        reportField('Source status', displayLabel(registrarRdapDiagnostics.status || registrarRdap.status)),
        reportField('Endpoint', registrarRdap.endpoint),
        reportField('Transport', displayLabel(registrarRdap.transportSecurity)),
        reportField('HTTP status', registrarRdap.upstreamStatus),
        reportField('Fetched', registrarRdap.fetchedAt),
        reportField('Domain', registrarRdapParsed.domain),
        reportField('Registrar', entityName(registrarRdapParsed.registrar)),
        reportField('Registrar IANA ID', registrarRdapParsed.registrarIanaId),
        reportField('Created', lifecycleValue(registrarRdapParsed, 'createdDate')),
        reportField('Expires', lifecycleValue(registrarRdapParsed, 'expiryDate')),
        reportField('Last updated', lifecycleValue(registrarRdapParsed, 'updatedDate')),
        reportField('DNSSEC', registrarRdapParsed.dnssec),
        reportField('Statuses', listText(registrarRdapParsed.statuses)),
        reportField('Name servers', listText(registrarRdapParsed.nameservers)),
        reportField('Server-declared truncation', yesNoUnknown(registrarRdapParsed.serverTruncated)),
      ],
    },
    whois: {
      title: 'WHOIS',
    ...
    networkRegistration: {
      ...
      fields: [
        ...
        reportField('Complete response', yesNoUnknown(networkContext.complete)),
        reportField('Truncated response', yesNoUnknown(networkContext.truncated)),
        reportField('Limitations', listText(networkContext.limitations)),
      ],
    },
  ...
  return {
    ...
    assessment: [
      ...
      reportField('Risk score', risk.score === null ? null : `${risk.score}/100`),
      reportField('Risk model', risk.modelVersion === null ? null : `v${risk.modelVersion}`),
      reportField('Risk factors', risk.factors.length ? listText(risk.factors) : risk.score === null ? null : 'None observed'),
...
export type {
  ComparisonField,
  LookupEvidenceReport,
  LookupEvidenceReportOptions,
  PublicationComparisonField,
  ReportField,
  ReportGroup,
  UnknownRecord,
};
