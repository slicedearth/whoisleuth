...
function normalizedExactBodyHash(value) {
  if (!value || typeof value !== 'object') return null;
  if (value.algorithm !== 'sha256' || !/^[a-f0-9]{64}$/i.test(String(value.value || ''))) return null;
  if (!['complete-body', 'captured-prefix'].includes(value.scope)) return null;
  if (!Number.isSafeInteger(value.bytes) || value.bytes < 0 || value.bytes > MAX_FINGERPRINT_SOURCE_BYTES) return null;
  return {
    algorithm: 'sha256',
...
function parseAttributes(tag) {
  ...
  if (start === -1) return { values: new Map(), truncated: false };
...
function decodeStableEntities(text) {
...
function reduceDynamicValues(value) {
  return value
...
function normalizeAttribute(name, value, baseUrl) {
...
function normalizedMarkupAndStructure(html, baseUrl) {
...
function visibleTextFingerprint(text) {
...
function formActionClass(value, baseUrl) {
...
function formStructureFingerprint(markup, baseUrl) {
  const forms = [];
  let current = null;
...
function normalizedResourceHosts(resources) {
...
function normalizedIdentifiers(identifiers, explicitlyTruncated = false) {
  const values = [];
  const seen = new Set();
...
function createPageFingerprints(html, options = {}) {
