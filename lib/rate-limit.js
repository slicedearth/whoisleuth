...
function trustsForwardedHeaders(env = process.env) {
  return Boolean(env && (env.NETLIFY === 'true' || env.NETLIFY === '1' || env.TRUST_PROXY === '1'));
}

function getForwardedProtocol(headers, env = process.env) {
...
function getClientIp(headers, fallback, env = process.env) {
  const h = headers || {};
