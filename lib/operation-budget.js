...
function normalizeUsageCeiling(value, label) {
  if (!Number.isSafeInteger(value) || value < 1 || value > MAX_OPERATION_USAGE_LIMIT) {
    throw new TypeError(`${label} must be a positive bounded integer`);
  }
  return value;
}

function normalizeOperationUsageLimits(policy) {
  let value = policy;
  ...
  const daily = normalizeUsageCeiling(value.daily, 'Daily operation limit');
  const monthly = normalizeUsageCeiling(value.monthly, '30-day operation limit');
  if (monthly < daily) throw new TypeError('30-day operation limit must not be lower than the daily limit');

  const rawFeatures = value.features === undefined ? {} : value.features;
  ...
  for (const [id, configured] of featureEntries) {
    ...
    const featureDaily = normalizeUsageCeiling(configured.daily, `Daily ${id} limit`);
    const featureMonthly = normalizeUsageCeiling(configured.monthly, `30-day ${id} limit`);
