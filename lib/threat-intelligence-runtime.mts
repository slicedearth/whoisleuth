...
function assertCuratedConnectorDefinition(
  connector: unknown,
): asserts connector is CuratedConnectorDefinition {
  if (
    !connector ||
    typeof connector !== 'object' ||
    !CURATED_CONNECTOR_DEFINITIONS.has(
      connector as CuratedConnectorDefinition,
    )
  ) {
    throw new TypeError('A versioned curated connector definition is required');
  }
