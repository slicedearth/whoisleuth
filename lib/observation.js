...
const STATUSES = new Set(['success', 'partial', 'not_found', 'skipped', 'error', 'unsupported', 'not_applicable']);
const SCAN_MODES = new Set(['fast', 'deep']);
...
function normalizeDiagnosticValue(value) {
  ...
  for (const field of ['status', 'error', 'detail', 'truncated', 'discarded', 'count']) {
    ...
    const normalized = normalizeDiagnosticValue(value[field]);
...
function normalizeDiagnostics(value) {
  ...
  const output = {};
  for (const key of Object.keys(value).sort().slice(0, MAX_DIAGNOSTICS)) {
    if (!key || key.length > MAX_DIAGNOSTIC_KEY || !/^[a-z0-9_-]+$/i.test(key)) continue;
    const normalized = normalizeDiagnosticValue(value[key]);
...
function createObservation(input = {}) {
  ...
  const limitations = [...new Set((Array.isArray(input.limitations) ? input.limitations : [])
    .map((item) => safeString(item, MAX_LIMITATION_LENGTH)).filter(Boolean))]
    .slice(0, MAX_LIMITATIONS);
  return {
    version: OBSERVATION_VERSION,
    status,
    observedAt,
    scanMode: SCAN_MODES.has(input.scanMode) ? input.scanMode : null,
    source,
...
function readObservationEnvelope(value) {
  ...
  if (Number.isInteger(value.version) && value.version > OBSERVATION_VERSION) return { state: 'unsupported', observation: null };
  if (value.version !== OBSERVATION_VERSION || !STATUSES.has(value.status) || !safeString(value.source, 40) || !isoTimestamp(value.observedAt)) {
    return { state: 'invalid', observation: null };
  }
  return { state: 'supported', observation: createObservation(value) };
}
