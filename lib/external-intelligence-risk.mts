...
type ExternalIntelligenceCalibration = {
  version: number;
  contribution: number;
  factor: { label: string; delta: number } | null;
...
export function calibrateExternalIntelligenceRisk(value: unknown): ExternalIntelligenceCalibration {
  ...
  const knownAges = sources.map((source) => source.ageDays).filter((age): age is number => age !== null);
  const independentPublisherCount = publisherFamilies.size;
  const recentPublisherCount = recentPublisherFamilies.size;

  let contribution = 0;
  let factor: { label: string; delta: number } | null = null;
...
export type { ExternalIntelligenceCalibration };
