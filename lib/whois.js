...
const { safeFetch, readTextCapped, resolvePublicAddresses } = require('./safe-fetch');
const { registryDateIso } = require('./registry-dates');
...
function queryWhoisAddress(address, server, query, {
  port = 43,
  timeoutMs = 10000,
...
async function whoisQuery(server, query, {
  ...
  for (const { address } of candidates) {
    ...
    try {
      ...
      failures.push(String(err && err.message ? err.message : 'connection failed').slice(0, 200));
    }
  }
...
async function fetchGtRegistryWhois(domain, { fetcher = safeFetch } = {}) {
...
function boundedWhoisValue(value, maxLength = MAX_WHOIS_FIELD_LENGTH) {
...
function addBoundedWhoisSetValue(set, rawValue, {
...
function normalizedWhoisContact(fields, prefix, role, truncatedFields) {
...
function parseWhoisChain(chain) {
  ...
  const expandedStreetFields = new Set();
