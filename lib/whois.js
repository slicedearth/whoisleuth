...
const { resolvePublicAddresses } = require('./safe-fetch');

const IANA_WHOIS = 'whois.iana.org';
const MAX_WHOIS_BYTES = 200000; // far more than even a large multi-section response needs
const WHOIS_TOTAL_DEADLINE_MS = 20000; // hard ceiling regardless of inactivity-timeout resets
...
function whoisQuery(server, query, { port = 43, timeoutMs = 10000 } = {}) {
  return resolvePublicAddresses(server).then(([{ address }]) => new Promise((resolve, reject) => {
    ...
    socket.setTimeout(timeoutMs); // inactivity timeout - resets on each chunk received
...
const NOT_FOUND_RE = /no match for|no match\b|not found|no data found|no entries found|domain not found|no object found|not registered|status:\s*(?:available|free)|is available for registration|available for purchase/i;
...
function classifyHopEvidence(hop, index) {
  ...
  // Positive evidence is checked before rate-limit/not-found so a genuine
  // registration record (hop 0 excepted - it describes the TLD, not the
  // domain) isn't misread as throttled because of stray boilerplate.
...
// report which hop settled it and whether any hop failed. `notFound` (the
// domain is genuinely available) is true only when the authoritative hop says
// so AND no hop produced positive registration evidence.
function analyzeWhoisChainAuthority(chain) {
  ...
  const positive = evidence.find((e) => e.kind === 'positive');
  ...
  if (!positive && authoritative && authoritative.kind === 'negative') {
    notFound = true;
    notFoundSource = authoritative.server;
  }

  const hasDefinitive = Boolean(positive) || (authoritative && authoritative.kind === 'negative');
  return {
    notFound,
    notFoundSource,
    authoritativeHop: positive ? positive.server : (authoritative ? authoritative.server : null),
    failedHop: failed.length ? failed[0].server : null,
    chainStatus: hasDefinitive && failed.length === 0 ? 'complete' : 'partial',
  };
}
