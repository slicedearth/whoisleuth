...
type SafeFetchHop = { url: string; status: number; location: string | null; durationMs: number };
type SafeFetchDetailedResult = {
  response: Response;
  requestedUrl: string;
  finalUrl: string;
  redirected: boolean;
...
async function resolvePublicAddresses(hostname: string): Promise<PublicAddressRecord[]> {
...
function normalizedFetchUrl(value: unknown): string {
...
async function safeFetchDetailed(
  url: string,
  options: RequestInit = {},
...
async function safeFetch(url: string, options: RequestInit = {}, redirectsLeft = MAX_REDIRECTS): Promise<Response> {
