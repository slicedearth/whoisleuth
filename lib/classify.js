...
function classifyQuery(raw) {
  ...
  // Strip a scheme, any path/query/fragment, and a single terminal root dot
  // (`example.com.` is the fully-qualified form of `example.com`).
  const q = trimmed.replace(/^[a-z]+:\/\//i, '').split(/[/?#]/)[0].replace(/\.$/, '');
