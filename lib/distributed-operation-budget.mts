...
type UsageLimits = Readonly<{
  modelVersion: number;
  daily: number;
  monthly: number;
  features: Readonly<Record<string, FeatureUsageLimit>>;
}>;
type RestRequest = (url: string, options: RequestInit, redirectsLeft?: number) => Promise<Response>;
type ResponseReader = (response: Response, maxBytes: number) => Promise<{ text: string; truncated: boolean }>;
type RestCommand = (body: unknown[]) => Promise<unknown>;
type DistributedDependencies = {
  safeFetch?: RestRequest;
  readTextCapped?: ResponseReader;
  command?: RestCommand;
...
type OperationContext = { operationFeature?: unknown };
type DenialScope = 'session' | 'runtime' | 'global_daily' | 'global_30_day' | 'feature_daily' | 'feature_30_day';
...
function normalizedRestUrl(value: unknown): string {
...
function normalizedNamespace(value: unknown = DEFAULT_NAMESPACE): string {
...
function normalizedLimits(value: unknown): OperationLimits {
  ...
  const limits: Record<string, OperationLimit> = {};
  for (const [id, configured] of entries) {
    ...
    limits[id] = Object.freeze({ session: limit.session as number, runtime: limit.runtime as number });
  }
  return Object.freeze(limits);
}

function normalizedUsageLimits(value: unknown): UsageLimits | null {
  ...
  if (!isSafeInteger(daily)
    || !isSafeInteger(monthly)
    || daily < 1
    || monthly < daily
    || monthly > MAX_USAGE_LIMIT) {
    throw new Error('Invalid distributed global usage limits');
  }
  const rawFeatures = record.features == null ? {} : record.features;
  ...
  for (const [id, configured] of entries) {
    const feature = configured && typeof configured === 'object' && !Array.isArray(configured)
      ? configured as Record<string, unknown>
      : null;
    const featureDaily = feature && feature.daily;
    const featureMonthly = feature && feature.monthly;
    if (!/^[a-z0-9_]{1,50}$/.test(id)
      || !isSafeInteger(featureDaily)
      || !isSafeInteger(featureMonthly)
      || featureDaily < 1
...
function createRestCommandClient({ url, token }: DistributedConfig, dependencies: DistributedDependencies = {}): RestCommand {
  ...
  return async function command(body: unknown[]): Promise<unknown> {
...
function classKeys(namespace: string, operationClass: string, sessionKey: string) {
...
function usageKeys(namespace: string, operationFeature: string) {
  return {
...
function normalizedOperationFeature(context: unknown): string {
  const value = context && typeof context === 'object' ? (context as OperationContext).operationFeature : null;
...
function providerUnavailable(operationClass: string) {
  return {
    allowed: false,
    operationClass,
    scope: 'provider',
...
function validResultArray(value: unknown, minimumLength: number): value is unknown[] {
  return Array.isArray(value)
    && value.length >= minimumLength
    && value.slice(0, minimumLength).every((item) => Number.isSafeInteger(Number(item)) && Number(item) >= 0);
}

function createDistributedOperationBudget(config: DistributedConfig, dependencies: DistributedDependencies = {}) {
  ...
  async function acquire(operationClass: string, sessionKey: unknown, context: OperationContext = {}) {
    ...
    if (!allowed) {
      ...
      const scopes: Record<number, DenialScope> = {
        1: 'session',
        2: 'runtime',
        3: 'global_daily',
        4: 'global_30_day',
