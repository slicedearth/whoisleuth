const { defaultOperationBudget, operationBudgetReport } = require('./operation-budget');
const {
...
function capabilityReport(runtime = 'unknown', env = process.env, operationBudget = defaultOperationBudget) {
  ...
  return {
    ...
    limitations: concurrency.distributed
      ? ['Operation concurrency uses deployment-wide distributed leases; fixed-window request rate limiting remains local to each runtime instance.']
