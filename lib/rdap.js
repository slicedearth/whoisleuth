...
function summarizeTextBlocks(blocks) {
  ...
  for (const block of blocks.slice(0, 50)) {
    ...
    const descriptions = [];
    for (const text of (Array.isArray(block.description) ? block.description : []).slice(0, 20)) {
      const description = truncatedText(text, 800);
      if (description) descriptions.push(description);
      if (descriptions.length >= 6) break;
    }
    if (!descriptions.length) continue;
    output.push({ title: truncatedText(block.title, 160) || 'Notice', descriptions });
  ...
  if (type === 'domain') {
    ...
    const nameserverDetails = Array.isArray(data.nameservers)
      ? data.nameservers.slice(0, 200).map((ns) => {
          ...
          return {
            ...
            ].slice(0, 20).map((address) => boundedString(address, 80)).filter(Boolean),
          };
    ...
    const dsData = secureDns && Array.isArray(secureDns.dsData)
      ? secureDns.dsData.slice(0, 50).map((ds) => {
          ...
          return {
            keyTag: boundedInteger(ds.keyTag, 0, 65535),
            algorithm: boundedInteger(ds.algorithm, 0, 255),
            digestType: boundedInteger(ds.digestType, 0, 255),
            digest: boundedString(ds.digest, 512),
          };
        }).filter(Boolean)
      : [];
    return {
    ...
    const cidrs = Array.isArray(data.cidr0_cidrs)
      ? data.cidr0_cidrs.slice(0, 200)
          .map((c) => {
            ...
            const prefix = boundedString(c.v4prefix || c.v6prefix, 80);
            const length = boundedInteger(c.length, 0, c.v4prefix ? 32 : 128);
