...
function normalizeSelectedDnsResolvers(value: unknown): string[] {
  ...
  if (!values.length || values.length > MAX_SELECTED_DNS_RESOLVERS || values.some((item) => isIP(item) === 0)) {
    throw new TypeError(`DNS resolver selection requires 1 to ${MAX_SELECTED_DNS_RESOLVERS} literal IPv4 or IPv6 addresses.`);
  }
  return values;
}
...
function createSelectedDnsResolvers(servers: readonly string[]): Record<string, DnsResolver> {
  if (!Array.isArray(servers) || !servers.length || servers.length > MAX_SELECTED_DNS_RESOLVERS
    || servers.some((item) => typeof item !== 'string' || isIP(item) === 0)) {
    throw new TypeError('Validated DNS resolver addresses are required.');
  }
