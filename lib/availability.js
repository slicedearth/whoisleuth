...
const { findRdapBase, rdapPathFor, findEntity, summarizeEntity } = require('./rdap');
const { buildWhoisChain, parseWhoisChain } = require('./whois');
const { safeFetch } = require('./safe-fetch');
const { checkMxRecords } = require('./dns-mx');
...
// a domain's homepage is untrusted content and could otherwise be an
...
async function fetchHomepageText(domain) {
  ...
  for (const scheme of ['https', 'http']) {
    ...
    try {
      ...
      if (res.ok) return await readTextCapped(res, MAX_HOMEPAGE_BYTES);
    } catch {
      clearTimeout(timeout);
    }
  }
