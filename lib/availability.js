...
async function fetchHomepage(domain) {
  ...
  for (const scheme of ['https', 'http']) {
    ...
    try {
      ...
      failures.push(`${scheme.toUpperCase()} returned HTTP ${res.status}`);
    } catch (err) {
...
async function checkDomainAvailability(domain, { fast = false } = {}) {
  ...
  const hasWhoisRegistrationData = Boolean(registrar) || Boolean(createdDate) || Boolean(expiryDate)
  ...
  if (homepage.status === 'fetched') {
    ...
    if (activityStatus === 'unknown') activityStatus = 'active';
  ...
  if (!forSaleSignal) {
    return {
      ...
      activityStatus,
      websiteProbeStatus: homepage.status,
      websiteProbeDetail: homepage.detail,
      faviconHash,
      faviconPHash,
      ...htmlSignals,
      ...mx,
