...
const { collectDnsIntelligence } = require('./dns-intelligence');
...
async function checkDomainAvailability(domain, options = {}) {
  ...
  const dnsDelegationPromise = !rdapFound
  ...
  if (!rdapFound && !hasWhoisRegistrationData && !dnsDelegated) {
    return {
      ...
      confidence: 'low',
      detail: fast
        ? 'No RDAP record or authoritative DNS delegation was available. A fast scan cannot determine registration status.'
        : whoisParsed && whoisParsed.failedHop
        ? `WHOIS was inconclusive - a referral hop did not answer conclusively (${whoisParsed.failedHop}).`
        : 'RDAP and WHOIS returned no conclusive registration data, and no authoritative DNS delegation was observed.',
      ...(fast ? {} : { source: 'whois' }),
    };
  }
