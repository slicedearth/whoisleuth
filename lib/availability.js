...
const { parseRegistryDate, registryDateIso } = require('./registry-dates');
...
async function fetchHomepage(domain, { fetcher = safeFetchDetailed } = {}) {
  ...
  for (const scheme of ['https', 'http']) {
    ...
    try {
      ...
      const fetchedResponse = /** @type {Response} */ (/** @type {unknown} */ (fetched));
      const detail = fetched && typeof fetched === 'object' && 'response' in fetched
        ? fetched
        : {
...
module.exports = {
  ...
  parseWhoisDate: parseRegistryDate,
};
