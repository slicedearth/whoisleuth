...
This complete reference documents every supported command contract. For a
short installation and first-use path, start with the [CLI guide](cli.md).
...
starting collection. It lists the normalised target, selected fast or deep
...
ceiling. These labels describe observable behaviour rather than granting
authorisation. The packaged `DISCLOSURE` defines the intended defensive scope;
operators remain responsible for law, provider terms, and authorisation.
...
The complete document also carries the normalised `rdap`, `whois`,
...
builder. Output retains normalised field states and observation times but no
...
has a reviewed disposition and retained normalised evidence. That local export
...
`lookalike-calibrate [dataset.json]` summarises reviewed candidate outcomes by
...
supported signed review artefact, it validates the declared SHA-256 digest. An
...
only successfully normalised domain names, while `--queries` preserves the
...
Terminal output summarises certificate rows, observed hostnames, canonical
...
output records the normalised custom selection. A custom dictionary file
...
one private, versioned local observation of the normalised seed, generation
...
states separately; version 1 is read and normalised on the next write. A later
...
DMARC authorisation, and dependency provenance. Warnings and dangers are
...
The command reports normalised redirect, response, content, selected security-
...
diagnostics, and normalised parsed source sections. Per-value, list, and event
bounds are revalidated because saved JSON is treated as untrusted input. A
successful registrar source must include a normalised object and agree with
...
servers. The shared comparison model normalises harmless case, punctuation,
...
capture package only when that additional active behaviour is authorised.
...
offline. Version 1 inputs remain supported. Version 2 adds optional
`customChecks` to retirement inputs and remains the current normalised output
version. The input uses `whoisleuth.domain-assurance.input` and one of three
fixed kinds:
...
- `recovery-dependencies` reviews up to 100 domains across registrar, DNS,
  ...
  concentration; and
- `retirement` checks the fixed decommissioning controls without treating an
  unchecked item as complete. Version 2 can add up to 20 analyst-defined checks,
  each with a bounded identifier, label, expected boolean, and true, false, or
  null observed value.
...
analyst deliberately shares an artefact. The analyst must select a TLP 2.0
...
supported artefact integrity where possible, detects stricter embedded TLP
markings, and reports only key-category counts plus bounded, control-free schema
and integer version metadata. It emits no content values and copies no raw
evidence.
...
control, legal advice, or recipient authorisation.
...
interpret a placeholder as a file path, read an artefact, make a request,
...
when the saved deep lookup represents it. Version 17 retains the normalised portable-field comparison, explicit
source-health states, a strict bounded projection of observed network
registration, an optional normalised security.txt disclosure source, HTTPS
...
Markdown output summarises query context, assessment state, registry sources,
...
after first reducing the typed Lookup response to the known normalised fields
needed by the report. It separately offers bounded IP and ASN summaries from
their normalised RDAP and source-health fields; IP reports can also include
